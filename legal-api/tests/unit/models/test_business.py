...
TEST_GOOD_STANDING_DATA = [
    (datetime.now() - datedelta.datedelta(months=6), LegalEntity.EntityTypes.COMP, LegalEntity.State.ACTIVE.value, True),
    (datetime.now() - datedelta.datedelta(months=6), LegalEntity.EntityTypes.COMP, LegalEntity.State.HISTORICAL.value, True),
    (datetime.now() - datedelta.datedelta(years=1, months=6), LegalEntity.EntityTypes.COMP, LegalEntity.State.ACTIVE.value, False),
    (datetime.now() - datedelta.datedelta(years=1, months=6), LegalEntity.EntityTypes.SOLE_PROP, LegalEntity.State.ACTIVE.value, True),
    (datetime.now() - datedelta.datedelta(years=1, months=6), LegalEntity.EntityTypes.PARTNERSHIP, LegalEntity.State.ACTIVE.value, True),
    (datetime.now() - datedelta.datedelta(months=6), LegalEntity.EntityTypes.SOLE_PROP, LegalEntity.State.ACTIVE.value, True),
    (datetime.now() - datedelta.datedelta(months=6), LegalEntity.EntityTypes.PARTNERSHIP, LegalEntity.State.ACTIVE.value, True)
]


@pytest.mark.parametrize('last_ar_date, entity_type, state, expected', TEST_GOOD_STANDING_DATA)
def test_good_standing(session, last_ar_date, entity_type, state, expected):
...
def test_business_json(session):
    ...
    d['dissolutionDate'] = datetime.date(legal_entity.dissolution_date).isoformat()
...
def test_continued_in_business(session):
    ...
    assert business_json['foreignIncorporationDate'] == datetime.date(
        LegislationDatetime.as_legislation_timezone(legal_entity.foreign_incorporation_date)
    ).isoformat()
