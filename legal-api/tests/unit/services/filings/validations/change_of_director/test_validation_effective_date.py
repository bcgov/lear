...
def test_validate_submission_date_not_before_founding(session):
    """Assert that the submission date cannot be a date prior to their Incorporation Date."""
    # setup
    identifier = 'CP1234567'
    now = datetime(2001, 8, 5, 12, 0, 0, 0, tzinfo=timezone.utc)
    business, filing = common_setup(identifier, now)

    # The submission date _cannot_ be before their Incorporation Date.
    before = datetime(2000, 8, 4, 12, 0, 0, 0, tzinfo=timezone.utc)
    filing['filing']['header']['date'] = before.isoformat()
    with freeze_time(now):
        err = validate_effective_date(business, filing)
    assert err

    # The submission date _can_ be on their Incorporation Date.
    on = datetime(2000, 8, 5, 12, 0, 0, 0, tzinfo=timezone.utc)
    filing['filing']['header']['date'] = on.isoformat()
    with freeze_time(now):
        err = validate_effective_date(business, filing)
    assert not err

    # The submission date _can_ be after their Incorporation Date.
    after = datetime(2000, 8, 6, 12, 0, 0, 0, tzinfo=timezone.utc)
    filing['filing']['header']['date'] = after.isoformat()
    with freeze_time(now):
        err = validate_effective_date(business, filing)
    assert not err
...
def test_validate_effective_date_not_before_other_COD(session):  # noqa: N802; COD is an acronym
    ...
    filing_cod['filing']['header']['date'] = now.isoformat()
    with freeze_time(now):
...
def test_validate_submission_date_not_before_other_COD(session):  # noqa: N802; COD is an acronym
    """Assert that the submission date of change cannot be before a previous COD filing."""
    # setup
    identifier = 'CP1234567'
    founding_date = datetime(2000, 8, 5, 12, 0, 0, 0, tzinfo=timezone.utc)
    business = Business(identifier=identifier,
    ...
    # The submission date _cannot_ be before the previous COD.
    before = datetime(2010, 8, 4, 12, 0, 0, 0, tzinfo=timezone.utc)
    filing_cod['filing']['header']['effectiveDate'] = as_effective_date(now).isoformat()
    filing_cod['filing']['header']['date'] = before.isoformat()
    with freeze_time(now):
        err = validate_effective_date(business, filing_cod)
    assert err

    # The submission date _can_ be on the same date as the previous COD.
    on = datetime(2010, 8, 5, 12, 0, 0, 0, tzinfo=timezone.utc)
    filing_cod['filing']['header']['effectiveDate'] = as_effective_date(now).isoformat()
    filing_cod['filing']['header']['date'] = on.isoformat()
    with freeze_time(now):
        err = validate_effective_date(business, filing_cod)
    assert not err

    # The submission date _can_ be after the previous COD.
    after = datetime(2010, 8, 6, 12, 0, 0, 0, tzinfo=timezone.utc)
    filing_cod['filing']['header']['effectiveDate'] = as_effective_date(now).isoformat()
    filing_cod['filing']['header']['date'] = after.isoformat()
    with freeze_time(now):
        err = validate_effective_date(business, filing_cod)
    assert not err
...
def test_validate_effective_date_not_before_other_AR_with_COD(session):  # noqa: N802; COD is an acronym
    ...
    filing_ar['filing']['header']['date'] = now.isoformat()
    with freeze_time(now):
...
def test_validate_submission_date_not_before_other_AR_with_COD(session):  # noqa: N802; COD is an acronym
    """Assert that the submission date of change cannot be before a previous AR filing."""
    # setup
    identifier = 'CP1234567'
    founding_date = datetime(2000, 8, 5, 12, 0, 0, 0, tzinfo=timezone.utc)
    business = Business(identifier=identifier,
    ...
    # The submission date _cannot_ be before the previous AR.
    before = datetime(2010, 8, 4, 12, 0, 0, 0, tzinfo=timezone.utc)
    filing_ar['filing']['header']['effectiveDate'] = as_effective_date(now).isoformat()
    filing_ar['filing']['header']['date'] = before.isoformat()
    with freeze_time(now):
        err = validate_effective_date(business, filing_ar)
    assert err

    # The submission date _can_ be on the same date as the previous AR.
    on = datetime(2010, 8, 5, 12, 0, 0, 0, tzinfo=timezone.utc)
    filing_ar['filing']['header']['effectiveDate'] = as_effective_date(now).isoformat()
    filing_ar['filing']['header']['date'] = on.isoformat()
    with freeze_time(now):
        err = validate_effective_date(business, filing_ar)
    assert not err

    # The submission date _can_ be after the previous AR.
    after = datetime(2010, 8, 6, 12, 0, 0, 0, tzinfo=timezone.utc)
    filing_ar['filing']['header']['effectiveDate'] = as_effective_date(now).isoformat()
    filing_ar['filing']['header']['date'] = after.isoformat()
    with freeze_time(now):
        err = validate_effective_date(business, filing_ar)
    assert not err
...
def as_effective_date(date_time: datetime) -> datetime:
    ...
    # 2. zero out the time
    # 3. convert back to a GMT datetime
    ...
    return LegislationDatetime.as_gmt_timezone(date_time)
