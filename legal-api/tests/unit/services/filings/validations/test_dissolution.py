...
import pytest
from registry_schemas.example_data import DISSOLUTION, FILING_HEADER, SPECIAL_RESOLUTION
...
def test_dissolution_type(
    session, test_status, entity_type, dissolution_type, identifier, expected_code, expected_msg
    ...
    filing["filing"]["dissolution"]["parties"][1]["deliveryAddress"] = filing["filing"]["dissolution"]["parties"][1][
        "mailingAddress"
    ]
...
def test_dissolution_statement_type(
    session,
    test_status,
    entity_type,
    dissolution_type,
    dissolution_statement_type,
    identifier,
    expected_code,
    expected_msg,
):  # pylint: disable=too-many-arguments
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("FAIL", "BC", "mailing_address_required", "BC1234567", HTTPStatus.BAD_REQUEST, "mailingAddress is required."),
        ("FAIL", "BC", "party_address_required", "BC1234567", HTTPStatus.BAD_REQUEST, "Dissolution party is required."),
        (
            "FAIL",
            "CP",
            "lookup_error",
...
def test_dissolution_address(
    session, test_status, entity_type, address_validation, identifier, expected_code, expected_msg
):  # pylint: disable=too-many-arguments
    """Assert that a VD can be validated."""
    # setup
    legal_entity = LegalEntity(identifier=identifier)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "FAIL_REQUIRED_SPECIAL_RESOLUTIONS",
            "CP",
            "voluntary",
            "CP1234567",
            False,
            HTTPStatus.BAD_REQUEST,
            [{"error": "Special Resolution is required.", "path": "/filing/specialResolution"}],
        ),
    ],
)
def test_dissolution_special_resolution(
    session,
    test_name,
    entity_type,
    dissolution_type,
    identifier,
    has_special_resolution_filing,
    expected_code,
    expected_msg,
):  # pylint: disable=too-many-arguments
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "FAIL_INVALID_AFFIDAVIT_FILE_KEY",
            "CP",
            "voluntary",
            "affidavitFileKey",
            "failAffidavit",
        ...
        (
            "FAIL_REQUIRED_AFFIDAVIT_FILE_KEY",
            "CP",
            "voluntary",
            "affidavitFileKey",
            "",
        ...
        (
            "FAIL_INVALID_AFFIDAVIT_FILE",
            "CP",
            "voluntary",
            "affidavitFileKey",
            "invalidAffidavitPageSize",
            "CP1234567",
            HTTPStatus.BAD_REQUEST,
            [
                {
...
def test_dissolution_affidavit(
    session,
    minio_server,
    test_name,
    entity_type,
