...
from legal_api.services.filings.validations.registrars_notation import validate
...
@pytest.mark.parametrize(
    ...
    [
        ('FAIL_INVALID_EFFECT_OF_ORDER', HTTPStatus.BAD_REQUEST, [
            {'error': 'Invalid effectOfOrder.', 'path': '/filing/registrarsNotation/effectOfOrder'}]),
...
def test_registrars_notation(session, test_status, expected_code, expected_msg):
    """Assert valid registrars notation."""
    ...
    if test_status == 'FAIL_INVALID_EFFECT_OF_ORDER':
        ...
        filing['filing']['registrarsNotation']['fileNumber'] = None
