...
def test_alteration_phone_number_validation(session, should_pass, phone_number, extension):
    """Test validate phone number and / or extension if they are provided."""
    identifier = 'BC1234567'
    legal_type = Business.LegalTypes.COMP.value
    changed_legal_type = Business.LegalTypes.BCOMP.value
    business = factory_business(identifier, entity_type=legal_type)

    filing = copy.deepcopy(ALTERATION_FILING_TEMPLATE)
    filing['filing']['header']['identifier'] = identifier
    filing['filing']['business']['legalType'] = legal_type
    filing['filing']['alteration']['business']['legalType'] = changed_legal_type
    del filing['filing']['alteration']['nameRequest']

    if phone_number:
        filing['filing']['alteration']['contactPoint']['phone'] = phone_number
    if extension:
        filing['filing']['alteration']['contactPoint']['extension'] = extension
