...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "No active consent continuation out for this date and/or jurisdiction.",
        ),
        ("SUCCESS", None, None),
    ],
)
def test_validate_continuation_out_date(session, test_name, expected_code, message):
    ...
    if test_name == "FAIL_IN_FUTURE":
        filing["filing"]["continuationOut"]["continuationOutDate"] = (
            LegislationDatetime.now() + datedelta.datedelta(days=1)
        ).strftime(date_format)
    elif test_name == "FAIL_NO_CCO":
        effective_date -= datedelta.datedelta(months=6, days=1)

    _create_consent_continuation_out(
        business, filing["filing"]["continuationOut"]["foreignJurisdiction"], effective_date
    )
