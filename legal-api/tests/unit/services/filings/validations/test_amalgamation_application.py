...
def test_has_future_effective_filing(mocker, app, session, jwt, test_status, expected_code, expected_msg):
    ...
    mocker.patch('legal_api.services.filings.validations.amalgamation_application.validate_name_request',
                 return_value=[])
    mocker.patch('legal_api.models.legal_entity.LegalEntity.find_by_identifier',
                 return_value=LegalEntity(identifier='BC1234567',
                                       entity_type=LegalEntity.EntityTypes.BCOMP.value))
...
def test_amalgamating_foreign_business_with_ulc_company(mocker, app, session, jwt,
    ...
    def mock_find_by_identifier(identifier):
        return LegalEntity(identifier=identifier,
                        ...
                        if test_status == 'FAIL' else LegalEntity.EntityTypes.BCOMP.value)
...
def test_amalgamating_cc_to_cc(mocker, app, session, jwt,
    ...
    def mock_find_by_identifier(identifier):
        return LegalEntity(identifier=identifier,
                        entity_type=LegalEntity.EntityTypes.BCOMP.value
                        if test_status == 'FAIL' else LegalEntity.EntityTypes.BC_CCC.value)
...
def test_amalgamating_expro_to_cc_or_ulc(mocker, app, session, jwt, test_status, legal_type):
    ...
    def mock_find_by_identifier(identifier):
        return LegalEntity(identifier=identifier,
                        entity_type=LegalEntity.EntityTypes.BC_CCC.value)
