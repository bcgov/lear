...
@pytest.mark.parametrize(
    "test_name, legal_type, delivery_region, delivery_country, \
        mailing_region, mailing_country, expected_code, expected_msg",
    [
...
def test_validate_amalgamation_office(
    session,
    mocker,
    test_name,
    legal_type,
...
def test_amalgamation_court_orders(
    mocker, app, session, test_status, file_number, effect_of_order, expected_code, expected_msg
):
...
@pytest.mark.parametrize(
    "test_status, expected_code, expected_msg",
    [("FAIL", HTTPStatus.BAD_REQUEST, ["BC1234567", "BC7654321"]), ("SUCCESS", None, None)],
)
def test_is_business_affliated(mocker, app, session, jwt, test_status, expected_code, expected_msg):
    ...
    if test_status == "SUCCESS":
        ...
        assert expected_code == err.code
        assert (
            f"{expected_msg[0]} is not affiliated with the currently selected BC Registries account."
            == err.msg[0]["error"]
        )
        assert (
            f"{expected_msg[1]} is not affiliated with the currently selected BC Registries account."
            == err.msg[1]["error"]
        )
...
@pytest.mark.parametrize(
    "test_status, expected_code, expected_msg",
    [("FAIL", HTTPStatus.BAD_REQUEST, "A business with identifier:BC7654321 not found."), ("SUCCESS", None, None)],
)
...
def test_amalgamating_foreign_business_with_bc_company_to_ulc(
    mocker, app, session, jwt, test_status, role, expected_code, expected_msg
):
...
def test_amalgamating_foreign_business_with_ulc_company(
    mocker, app, session, jwt, test_status, role, expected_code, expected_msg
):
...
def test_amalgamating_cc_to_cc(mocker, app, session, jwt, test_status, expected_code, expected_msg):
...
@pytest.mark.parametrize(
    ...
    [
        ("FAIL", LegalEntity.EntityTypes.BC_CCC.value),
        ("SUCCESS_CC", LegalEntity.EntityTypes.BC_CCC.value),
        ("FAIL", LegalEntity.EntityTypes.BC_ULC_COMPANY.value),
        ("SUCCESS_ULC", LegalEntity.EntityTypes.BC_ULC_COMPANY.value),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            HTTPStatus.BAD_REQUEST,
            "Duplicate amalgamating business entry found in list: BC1234567.",
        ),
        (
            "FAIL_EXPRO",
            [
            ...
            HTTPStatus.BAD_REQUEST,
            "Duplicate amalgamating business entry found in list: 123456.",
        ),
        (
            "SUCCESS",
            [
...
def test_duplicate_amalgamating_businesses(
    mocker, app, session, jwt, test_status, amalgamating_businesses, expected_code, expected_msg
):
    ...
    [
        ...
        (
            ...
            [
                {"role": AmalgamatingBusiness.Role.amalgamating.name, "identifier": "BC1234567"},
                {
                    "role": AmalgamatingBusiness.Role.holding.name,
                    "identifier": "1234568",
                    "legalName": "Foreign Co.",
...
def test_amalgamating_business_roles(
    mocker, app, session, jwt, amalgamation_type, amalgamating_businesses, expected_code, expected_msg
):
...
def test_horizontal_amalgamation(mocker, app, session, jwt, test_name, expected_code, expected_msg):
    ...
    filing["filing"]["amalgamationApplication"]["amalgamatingBusinesses"][0][
        "role"
