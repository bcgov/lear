...
def create_party(roles: list, party_id=None, mailing_addr=None, delivery_addr=None, officer=None):
    """Create party object with custom roles, mailing addresses and delivery aadresses."""

    if officer:
        party = {"officer": officer, "mailingAddress": None, "deliveryAddress": None, "roles": []}
        if party_id:
            party.id = party_id
    else:
        party = {
    ...
    for role in roles:
        party["roles"].append({"roleType": role, "appointmentDate": "2018-01-01"})
...
def create_party_address(
    base_address=None, street=None, street_additional=None, city=None, country=None, postal_code=None, region=None
):
    ...
    if base_address:
        ...
        party_address["streetAddressAdditional"] = (
            street_additional if street_additional is not None else party_address["streetAddressAdditional"]
        )
...
def create_officer(
    base_officer=None, first_name=None, middle_name=None, last_name=None, organization_name=None, party_type=None
):
    if base_officer:
        ...
        base_officer["organizationName"] = (
            organization_name if organization_name is not None else officer["organizationName"]
        )
