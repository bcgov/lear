...
def factory_limited_restoration_filing(approval_type: str = "courtOrder"):
    filing = Filing(_filing_type="restoration", _filing_sub_type="limitedRestoration", approval_type=approval_type)
    return filing


def execute_test_restoration_nr(
    mocker, filing_sub_type, entity_type, nr_number, nr_type, new_legal_name, expected_code, expected_msg
):
    ...
    legal_entity = LegalEntity(
        identifier="BC1234567", _entity_type=entity_type, restoration_expiry_date=LegislationDatetime.now()
    )
    ...
    if not nr_number and not new_legal_name:
        ...
        filing["filing"]["restoration"]["nameRequest"]["legalType"] = entity_type
        if nr_number:
    ...
    temp_nr_response["legalType"] = entity_type
    ...
    with patch.object(Filing, "get_a_businesses_most_recent_filing_of_a_type", return_value=limited_restoration_filing):
...
def test_validate_party(session, test_name, party_role, expected_msg):
    ...
    legal_entity = LegalEntity(identifier="BC1234567", _entity_type="BC")
    filing = copy.deepcopy(FILING_HEADER)
    filing["filing"]["restoration"] = copy.deepcopy(RESTORATION)
...
def test_validate_relationship(session, test_status, restoration_type, expected_code, expected_msg):
    ...
    legal_entity = LegalEntity(
        identifier="BC1234567", _entity_type="BC", restoration_expiry_date=LegislationDatetime.now()
    )
    limited_restoration_filing = None
...
def test_validate_expiry_date(session, test_name, restoration_type, delta_date, is_valid):
    ...
    legal_entity = LegalEntity(
        identifier="BC1234567", _entity_type="BC", restoration_expiry_date=restoration_expiry_date
    )
...
@pytest.mark.parametrize(
    "test_status, restoration_types, entity_types, approval_type, limited_restoration_approval_type, expected_code,\
        expected_msg",
    [
        (
            "FAIL",
            ["fullRestoration", "limitedRestoration", "limitedRestorationExtension", "limitedRestorationToFull"],
            ["BC", "BEN", "ULC", "CC"],
            None,
            "courtOrder",
        ...
        (
            ...
            "Must provide approval type with value of registrar.",
        ),
        (
            "FAIL",
            ["limitedRestorationExtension", "limitedRestorationToFull"],
            ...
            "courtOrder",
            HTTPStatus.BAD_REQUEST,
            "Must provide approval type with value of courtOrder.",
        ),
        (
            "SUCCESS",
            ["fullRestoration", "limitedRestoration", "limitedRestorationExtension", "limitedRestorationToFull"],
...
def test_approval_type(
    ...
    for restoration_type in restoration_types:
        ...
        for entity_type in entity_types:
            ...
            with patch.object(
                Filing, "get_a_businesses_most_recent_filing_of_a_type", return_value=limited_restoration_filing
            ):
                err = validate(legal_entity, filing)
...
@pytest.mark.parametrize(
    ...
    [
        (
            "FAIL",
            ["fullRestoration", "limitedRestoration"],
            ["BC", "BEN", "ULC", "CC"],
            "courtOrder",
            None,
            None,
            HTTPStatus.BAD_REQUEST,
            "Must provide Court Order Number.",
        ),
        (
            "FAIL",
            ["limitedRestorationExtension", "limitedRestorationToFull"],
...
def test_restoration_court_orders(
    session,
    test_status,
    restoration_types,
    legal_types,
    ...
    for restoration_type in restoration_types:
        ...
        for legal_type in legal_types:
            legal_entity = LegalEntity(
                identifier="BC1234567", _entity_type=legal_type, restoration_expiry_date=LegislationDatetime.now()
            )
            filing = copy.deepcopy(FILING_HEADER)
            filing["filing"]["restoration"] = copy.deepcopy(RESTORATION)
            filing["filing"]["header"]["name"] = "restoration"
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "2023-03-30",
            HTTPStatus.BAD_REQUEST,
            "Must provide notice of application mailed date.",
        ),
        (
            "FAIL",
            ["fullRestoration", "limitedRestoration"],
            ...
            None,
            HTTPStatus.BAD_REQUEST,
            "Must provide BC Gazette published date.",
        ),
        (
            "SUCCESS",
            ["fullRestoration", "limitedRestoration"],
...
def test_restoration_registrar(
    session,
    test_status,
    restoration_types,
    legal_types,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "new name",
            HTTPStatus.BAD_REQUEST,
            "Cannot change name while extending limited restoration.",
        ),
...
def test_restoration_nr(
    session, mocker, test_status, filing_sub_type, legal_types, nr_number, new_legal_name, expected_code, expected_msg
):
    """Assert nr block of filing is validated correctly."""

    for legal_type in legal_types:
        execute_test_restoration_nr(
            mocker, filing_sub_type, legal_type, nr_number, None, new_legal_name, expected_code, expected_msg
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "new name",
            HTTPStatus.BAD_REQUEST,
            "The name type associated with the name request number entered cannot be used.",
        ),
        # limited restoration
        (
            "SUCCESS_NR_TYPE",
        ...
        (
            ...
            ["BC", "BEN", "ULC", "CC"],
            None,
            ["RCC", "RCR", "BERE", "RUL", "RCRC", "BEREE", "RULL"],
            None,
            None,
            None,
        ),
...
def test_restoration_nr_type(
    ...
    for legal_type in legal_types:
        for nr_type in nr_types:
            execute_test_restoration_nr(
                mocker, filing_sub_type, legal_type, nr_number, nr_type, new_legal_name, expected_code, expected_msg
            )
