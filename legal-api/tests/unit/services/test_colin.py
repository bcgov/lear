...
def test_call_colin_api(app, requests_mock, system_token):
    ...
    response = ColinService.call_colin_api(PUBLIC_PATH)
...
def test_call_colin_api_with_given_token(app, requests_mock, system_token):
    ...
    response = ColinService.call_colin_api(PUBLIC_PATH, token='user-token')
...
def test_call_colin_api_without_token(app, requests_mock, mocker):
    ...
    assert ColinService.call_colin_api(PUBLIC_PATH) is None
    assert not colin_mock.called


def test_call_colin_api_token_failure(app, mocker):
    """Assert a token-service outage returns None instead of raising."""
...
def test_call_colin_api_connection_failure(app, requests_mock, system_token):
    """Assert a colin connection failure returns None instead of raising."""
...
def test_call_colin_api_caches_definitive_responses(app, requests_mock, system_token):
    ...
    assert ColinService.call_colin_api(PUBLIC_PATH).status_code == HTTPStatus.OK
    assert ColinService.call_colin_api(PUBLIC_PATH).status_code == HTTPStatus.OK
    # a 404 is a definitive answer too
    assert ColinService.call_colin_api('businesses/BC0000000/public').status_code == HTTPStatus.NOT_FOUND
    assert ColinService.call_colin_api('businesses/BC0000000/public').status_code == HTTPStatus.NOT_FOUND
...
def test_call_colin_api_does_not_cache_failures(app, requests_mock, system_token):
    ...
    assert ColinService.call_colin_api(PUBLIC_PATH).status_code == HTTPStatus.INTERNAL_SERVER_ERROR
    assert ColinService.call_colin_api(PUBLIC_PATH).status_code == HTTPStatus.INTERNAL_SERVER_ERROR
...
def test_query_business(app, requests_mock, system_token):
    ...
    response = ColinService.query_business('BC0870226')
...
def test_get_snapshot(app, requests_mock, system_token):
    ...
    response = ColinService.get_snapshot('BC0870226')

    assert response.status_code == HTTPStatus.OK
    assert response.json()['business']['identifier'] == 'BC0870226'
    # repeat calls are served from the cache
    assert ColinService.get_snapshot('BC0870226').status_code == HTTPStatus.OK
    assert snapshot_mock.call_count == 1
    # but a caller can opt out of the cache to force a fresh fetch
    assert ColinService.get_snapshot('BC0870226', use_cache=False).status_code == HTTPStatus.OK
