...
def factory_party_organization(organization_name: str, identifier: str, custom_id: int = None) -> ColinEntity:
    party = ColinEntity(organization_name=organization_name, identifier=identifier)
...
def factory_party_role_person(
    role: EntityRole.RoleTypes, cessation_date: datetime = None, legal_entity=None, custom_person_id: int = None
):
...
def factory_party_role_organization(
    role: EntityRole.RoleTypes, cessation_date: datetime = None, legal_entity=None, custom_org_id: int = None
):
    party_role = EntityRole(role_type=role, cessation_date=cessation_date)
    if legal_entity:
        party_role.legal_entity = legal_entity
...
def factory_filing_role_person(
    filing_id: int, role: str, cessation_date: datetime = None, custom_person_id: int = None
):
...
def factory_filing_role_organization(
    filing_id: int, role: str, cessation_date: datetime = None, custom_org_id: int = None
):
    party_role = EntityRole(filing_id=filing_id, role_type=role, cessation_date=cessation_date)
    if custom_org_id:
        related_colin_entity = factory_party_organization(TEST_BUSINESS_NAME, "FM1112222", custom_org_id)
...
def factory_party_roles(
    ...
    for idx in range(num_persons_roles):
        ...
        person_role = factory_party_role_person(
            legal_entity=legal_entity, role=role, cessation_date=person_cessation_date
        )
    ...
    for idx in range(num_org_roles):
        ...
        org_role = factory_party_role_organization(
            legal_entity=legal_entity, role=role, cessation_date=org_cessation_date
        )
...
def factory_address(
    ...
    address = Address(
        address_type=address_type, street=street, city=city, country=country, postal_code=postal_code, region=region
    )
...
def create_business(
    ...
    if firm_num_persons_roles > 0 or firm_num_org_roles > 0:
        ...
        firm_entity_roles = factory_party_roles(
            firm_entity_role, firm_num_persons_roles, firm_num_org_roles, person_cessation_dates, org_cessation_dates
        )
