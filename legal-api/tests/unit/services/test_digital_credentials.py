...
def test_data_helper(app, session, test_data):
    ...
    business = factory_legal_entity(**test_data["business"])
    ...
    for party_role in test_data["party_roles"]:
        _party_role = EntityRole(**party_role)
        _party_role.legal_entity_id = business.id
        _party_role.save()

    issued_business_user_credential = DCIssuedBusinessUserCredential(legal_entity_id=business.id, user_id=user.id)
