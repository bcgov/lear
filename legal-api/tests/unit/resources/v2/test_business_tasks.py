...
AR_FILING_CURRENT_YEAR = {
    "filing": {
        "header": {"name": "annualReport", "date": "2019-08-13", "certifiedBy": "full name"},
        "business": {
            "cacheId": 1,
...
def test_bcorps_get_tasks_pending_filings(session, client, jwt):
    ...
    with nested_session(session):
        ...
        filing["filing"]["annualReport"]["annualReportDate"] = str(
            (datetime.today() - datedelta.datedelta(years=2)).date()
        )
...
def test_get_tasks_pending_correction_filings(session, client, jwt):
    ...
    with nested_session(session):
        ...
        with freeze_time(FROZEN_2018_DATETIME):
            # test endpoint returned filing in tasks call
            rv = client.get(
                f"/api/v2/businesses/{identifier}/tasks", headers=create_header(jwt, [STAFF_ROLE], identifier)
            )
            assert rv.status_code == HTTPStatus.OK
            assert len(rv.json["tasks"]) == 3
            assert rv.json["tasks"][0]["task"]["filing"]["header"]["filingId"] == filing.id
...
def test_construct_task_list(
    session, client, jwt, test_name, identifier, founding_date, previous_ar_date, entity_type, tasks_length
):
...
def test_conversion_filing_task(
    ...
    with nested_session(session):
        ...
        if conversion_task_expected:
            conversion_to_do = any(
                x["task"]["todo"]["header"]["name"] == "conversion" and x["task"]["todo"]["header"]["status"] == "NEW"
