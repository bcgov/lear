...
@pytest.mark.parametrize('test_name, identifier, founding_date, previous_ar_date, entity_type, tasks_length', [
    ...
    ('ULC current AR year issued', 'BC1234567', '1900-07-01', '2022-03-03',
     LegalEntity.EntityTypes.BC_ULC_COMPANY.value, 0),
...
def test_construct_task_list(session, client, jwt, test_name, identifier, founding_date, previous_ar_date, entity_type,
                             tasks_length):
    ...
    with nested_session(session):
        ...
        with patch('legal_api.resources.v2.business.business_tasks.check_warnings', return_value=[]):
            ...
            legal_entity =factory_legal_entity(
                identifier, founding_date, previous_ar_datetime, entity_type)
...
@pytest.mark.parametrize('test_name, entity_type, identifier, has_missing_business_info, conversion_task_expected', [
...
def test_conversion_filing_task(session, client, jwt, test_name, entity_type, identifier, has_missing_business_info,
