...
def test_get_business_share_classes(app, session, client, jwt, requests_mock, test_name, role):
    ...
    with nested_session(session):
        ...
        rv = client.get(
            f"/api/v2/businesses/{identifier}/share-classes", headers=create_header(jwt, [role], identifier)
        )
        # check
...
def test_get_business_no_share_classes(session, client, jwt):
    ...
    with nested_session(session):
        ...
        # test
        rv = client.get(
            f"/api/v2/businesses/{identifier}/share-classes", headers=create_header(jwt, [STAFF_ROLE], identifier)
        )
        # check
        assert rv.status_code == HTTPStatus.OK
...
def test_get_business_share_class_by_id(session, client, jwt):
    ...
    with nested_session(session):
        ...
        # test
        rv = client.get(
            f"/api/v2/businesses/{identifier}/share-classes/{share_class.id}",
...
def test_get_business_share_class_by_invalid_id(session, client, jwt):
    ...
    with nested_session(session):
        ...
        # test
        rv = client.get(
            f"/api/v2/businesses/{identifier}/share-classes/{share_class_id}",
            headers=create_header(jwt, [STAFF_ROLE], identifier),
        )
        # check
...
def test_get_share_classes_unauthorized(app, session, client, jwt, requests_mock):
    ...
    with nested_session(session):
        ...
        # test
        rv = client.get(
            f"/api/v2/businesses/{identifier}/share-classes", headers=create_header(jwt, [PUBLIC_USER], identifier)
        )
        # check
