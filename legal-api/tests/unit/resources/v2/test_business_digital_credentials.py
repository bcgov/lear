...
iXSwgImxhYmVsIjogImZhYmVyLmFnZW50IiwgInNlcnZpY2VFbmRwb2ludCI6ICJodHRwOi8vMTkyLjE2OC42NS4zOjgwMjAifQ=="""
        ...
        with patch.object(
            ...
            "create_invitation",
            return_value={"connection_id": connection_id, "invitation_url": invitation_url},
        ):
            rv = client.post(
                f"/api/v2/businesses/{identifier}/digitalCredentials/invitation",
                headers=headers,
...
def test_get_connection_not_found(session, client, jwt):  # pylint:disable=unused-argument
    ...
    with nested_session(session):
        ...
        rv = client.get(
            f"/api/v2/businesses/{identifier}/digitalCredentials/connection", headers=headers, content_type=content_type
        )
...
def test_send_credential(session, client, jwt):  # pylint:disable=unused-argument
    ...
    with nested_session(session):
        ...
        with patch.object(
            DigitalCredentialsService,
            "issue_credential",
            return_value={"credential_exchange_id": "3fa85f64-5717-4562-b3fc-2c963f66afa6"},
        ):
            rv = client.post(
...
def test_webhook_connections_notification(session, client, jwt):  # pylint:disable=unused-argument
    ...
    with nested_session(session):
        ...
        json_data = {"connection_id": connection.connection_id, "state": "active"}
        rv = client.post(
            "/api/v2/digitalCredentials/topic/connections", json=json_data, headers=headers, content_type=content_type
        )
        assert rv.status_code == HTTPStatus.OK
...
def test_webhook_issue_credential_notification(session, client, jwt):  # pylint:disable=unused-argument
    ...
    with nested_session(session):
        ...
        json_data = {"credential_exchange_id": issued_credential.credential_exchange_id, "state": "credential_issued"}
        rv = client.post(
            "/api/v2/digitalCredentials/topic/issue_credential",
            json=json_data,
            headers=headers,
            content_type=content_type,
        )
