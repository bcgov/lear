...
iXSwgImxhYmVsIjogImZhYmVyLmFnZW50IiwgInNlcnZpY2VFbmRwb2ludCI6ICJodHRwOi8vMTkyLjE2OC42NS4zOjgwMjAifQ=="""
    ...
    with patch.object(
        ...
        "create_invitation",
        return_value={"invitation": {"@id": invitation_id}, "invitation_url": invitation_url},
    ):
        rv = client.post(
            f"/api/v2/businesses/{identifier}/digitalCredentials/invitation", headers=headers, content_type=content_type
        )
...
def test_get_connections_not_found(app, session, client, jwt):  # pylint:disable=unused-argument
    ...
    rv = client.get(
        f"/api/v2/businesses/{identifier}/digitalCredentials/connections", headers=headers, content_type=content_type
    )
    assert rv.status_code == HTTPStatus.OK
...
def test_webhook_issue_credential_notification(app, session, client, jwt):  # pylint:disable=unused-argument
    ...
    json_data = {"cred_ex_id": issued_credential.credential_exchange_id, "state": "done"}
    rv = client.post(
        "/api/v2/digitalCredentials/topic/issue_credential_v2_0",
        json=json_data,
        headers=headers,
        content_type=content_type,
    )
