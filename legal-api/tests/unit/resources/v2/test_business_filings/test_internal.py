...
def test_colin_filing_to_queue(app_ctx, session, client, jwt, stan_server):
    ...
    with nested_session(session):
        ...
        this_loop.run_until_complete(
            queue.stan.subscribe(subject=queue.subject, queue="colin_queue", durable_name="colin_queue", cb=cb)
        )
...
@pytest.mark.parametrize(
    "identifier, base_filing, corrected_filing, colin_id",
    [
...
def test_patch_internal_filings(session, client, jwt):
    ...
    with nested_session(session):
        ...
        # make request
        rv = client.patch(
            f"/api/v2/businesses/internal/filings/{filing.id}",
...
def test_post_colin_last_update(session, client, jwt):
    ...
    with nested_session(session):
        rv = client.post(
            f"/api/v2/businesses/internal/filings/colin_id/{colin_id}", headers=create_header(jwt, [COLIN_SVC_ROLE])
        )
        assert rv.status_code == HTTPStatus.CREATED
        assert rv.json == {"maxId": colin_id}
