...
def test_get_all_business_filings_multi_in_ledger(session, client, jwt):
    ...
    with nested_session(session):
        ...
        for i in range(0, 3):
            ar["filing"]["annualReport"]["annualGeneralMeetingDate"] = datetime.date(
                add_years(datetime(2001, 8, 5, 7, 7, 58, 272362), i)
            ).isoformat()
            factory_filing(b, ar)
...
def test_ledger_search(session, client, jwt):
    ...
    with nested_session(session):
        ...
        # test
        rv = client.get(
            f"/api/v2/businesses/{identifier}/filings", headers=create_header(jwt, [UserRoles.system], identifier)
        )
...
def test_ledger_display_alteration_report(session, client, jwt):
    ...
    with nested_session(session):
        ...
        identifier = "BC1234567"
        today = date.today().isoformat()
        alteration_meta = {"alteration": {"fromLegalType": "BC", "toLegalType": "BEN"}}
...
def test_ledger_display_incorporation(session, client, jwt, test_name, identifier, entity_type, expected_display_name):
    ...
    with nested_session(session):
        ...
        ia_meta = {
            ...
            filing_name: {"nrNumber": nr_number, "businessName": business_name},
        }
...
def test_ledger_display_corrected_incorporation(session, client, jwt):
    ...
    with nested_session(session):
        ...
        correction = ledger_element_setup_filing(
            legal_entity, "correction", filing_date=legal_entity.founding_date + datedelta.datedelta(months=3)
        )
...
def test_ledger_redaction(
    ...
    with nested_session(session):
        ...
        try:
            ...
            filing_submission = {
                "filing": {
                    "header": {"name": filing_name, "date": "2019-04-08"},
                    filing_name: {"resolution": "Year challenge is hitting oppo for the win."},
                }
            }
            ...
            rv = client.get(
                f"/api/v2/businesses/{identifier}/filings", headers=create_header(jwt, [jwt_role], identifier)
            )
