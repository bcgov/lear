...
from legal_api.models import LegalEntity, RegistrationBootstrap, EntityRole
...
def test_business_filing_ia_parties(session, client, jwt):
    ...
    b.entity_roles.append(party_role)
...
def _fee_code_asserts(legal_entity, filing_json: dict, multiple_fee_codes, expected_fee_code: str):
    """Assert fee codes."""
    fee_codes = ListFilingResource._get_filing_types(legal_entity, filing_json)
    assert fee_codes
    if len(fee_codes) == 1:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ('BC1234567', DISSOLUTION_VOLUNTARY_FILING, 'dissolution', LegalEntity.EntityTypes.LIMITED_CO.value, None,
            False, []),
        ('CP1234567', SPECIAL_RESOLUTION_NO_CON_FILING, 'specialResolution', LegalEntity.EntityTypes.COOP.value, None, False,
         []),
        ('CP1234567', CP_SPECIAL_RESOLUTION_TEMPLATE, 'specialResolution', LegalEntity.EntityTypes.COOP.value,
...
def test_get_correct_fee_codes(
    ...
    _fee_code_asserts(legal_entity, filing, multiple_fee_codes, expected_fee_code)
