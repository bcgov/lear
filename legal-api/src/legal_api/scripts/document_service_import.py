...
def import_documents(business_identifier):
    ...
    if business_identifier:
        ...
        if business is None:
            current_app.logger.info(f"Business {business_identifier} not found")
            sys.exit(1)
    ...
    for filing in colin_filings:
        ...
        for event_id in colin_event_id:
            ...
            headers = {"X-ApiKey": api_key, "Account-Id": account_id, "Authorization": "Bearer " + token}
            response = requests.get(url=req_url, headers=headers)
            if response.status_code == 200:
                for report in response.json():
                    document = Document.find_one_by(filing.business_id, filing.id, report["reportType"])
                    if document is None:
                        imported += 1
