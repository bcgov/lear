...
class Report:  # pylint: disable=too-few-public-methods, too-many-lines
    ...
    def _get_report_filename(self):
        ...
        return f"{legal_entity_number}_{filing_date}_{description}.pdf".replace(" ", "_")
    ...
    def _format_filing_json(self, filing):  # noqa: PLR0912
        ...
        elif self._report_key in {"letterOfConsent", "letterOfConsentAmalgamationOut", "continueOutApplication"}:
    ...
    def _format_consent_continuation_amalgamation_out_data(self, filing):
        ...
        filing["offices"] = VersionedBusinessDetailsService.\
            get_office_revision(self._filing.id, self._filing.transaction_id, self._business.id)
            ...
            for party in filing.get("parties"):
                if party_id := party["officer"].get("id"):
                    ...
                    if self._compare_address(party.get("deliveryAddress"), prev_party_json.get("deliveryAddress")):
                        party["deliveryAddress"]["changed"] = True
                        filing["partyChange"] = True
                elif [role for role in party.get("roles", []) if role["roleType"].lower() in ["director"]]:
                    filing["newParties"].append(party)
    ...
    def _set_meta_info(self, filing):
        ...
        if self._report_key == "noticeOfArticles":
            filing["meta_subject"] = f"{self._business.legal_name} ({self._business.identifier})"
        else:
...
class ReportMeta:  # pylint: disable=too-few-public-methods
    ...
    reports = {
        ...
        "continueOutApplication": {
            # FILING-2 is not used here: legacy consent filings stored the Letter of Consent
            # in the DRS as FILING-2 before it moved to FILING-3.
            "filingDescription": "Continue Out Application",
            "fileName": "continueOutApplication",
            "reportType": ReportTypes.FILING_4.value
        },
