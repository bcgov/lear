...
class Report:  # pylint: disable=too-few-public-methods, too-many-lines
    ...
    def _get_report(self):
        if flags.is_on("enable-document-records"):
            ...
            if account_id is not None and self._business is not None:
                document, status = self._document_service.get_document(
                    self._business.identifier, self._filing.id, self._report_key, account_id
                )
                if status == HTTPStatus.OK:
                    return current_app.response_class(response=document, status=status, mimetype="application/pdf")
        ...
        if flags.is_on("enable-document-records"):
            ...
            create_filing_types = ["incorporationApplication", "continuationIn", "amalgamation", "registration"]
            if self._filing.filing_type in create_filing_types:
                ...
                create_document = create_document and self._filing.status in (
            ...
            if create_document:
                self._document_service.create_document(
                    self._business.identifier, self._filing.id, self._report_key, account_id, response.content
                )
    ...
    def _format_transition_data(self, filing):
        ...
        if filing.get("shareStructure", {}).get("shareClasses", None):
            ...
            formatted_dates = [datetime.fromisoformat(date).strftime(OUTPUT_DATE_FORMAT) for date in dates]
    ...
    def _format_receiver_data(self, filing):
        ...
        all_receivers = PartyRole.get_party_roles(
            self._business.id, datetime.utcnow().date(), PartyRole.RoleTypes.RECEIVER.value
        )
        ...
        for receiver in all_receivers:
            if receiver.cessation_date is None:
                ...
                receiver_json["cessationDate"] = (
                    receiver.cessation_date.strftime(OUTPUT_DATE_FORMAT) if receiver.cessation_date else None
                )
        ...
        for receiver in receivers:
            for role in receiver["roles"]:
                ...
                if role.get("cessationDate"):
                    cessation_date = LegislationDatetime.as_legislation_timezone_from_date_str(role["cessationDate"])
        ...
        if self._filing.filing_sub_type == "limitedRestorationToFull":
            business_previous_restoration_expiry = (
                VersionedBusinessDetailsService.find_last_value_from_business_revision(
                    self._filing, is_restoration_expiry_date=True
                )
            )
    ...
    def _format_consent_continuation_amalgamation_out_data(self, filing):
        ...
        filing["offices"] = VersionedBusinessDetailsService.get_office_revision(
            self._filing.id, self._filing.transaction_id, self._business.id
        )
    ...
    def _set_amalgamating_businesses(self, filing):
        ...
        for amalgamating_business in amalgamating_businesses:
            ...
            ting_businesses.append({"legalName": business_legal_name, "identifier": identifier})
    ...
    def _get_versioned_amalgamating_business(self, identifier):
        ...
        if self._filing.transaction_id:
            ...
            ting_business = VersionedBusinessDetailsService.get_business_revision_obj(self._filing, ting_business.id)
    ...
    def _set_from_primary_or_holding_business_data(self, filing):  # pylint: disable=too-many-locals, too-many-branches
        ...
        if self._filing.transaction_id:
            parties_version = VersionedBusinessDetailsService.get_party_role_revision(
                self._filing, primary_or_holding_business.id, role=PartyRole.RoleTypes.DIRECTOR.value
            )
        ...
        if self._filing.transaction_id:
            offices = VersionedBusinessDetailsService.get_office_revision(
                self._filing.id, self._filing.transaction_id, primary_or_holding_business.id
            )
        else:
    ...
    def _format_change_of_registration_data(self, filing, filing_type):  # noqa: E501 # pylint: disable=too-many-locals, too-many-branches, too-many-statements
        ...
        if business_office := filing.get(filing_type).get("offices", {}).get("businessOffice"):
            ...
            offices_json = VersionedBusinessDetailsService.get_office_revision(
                prev_completed_filing.id, prev_completed_filing.transaction_id, self._filing.business_id
            )
    ...
    def _has_party_name_change(prev_party_json, current_party_json):
        ...
        if officer.get("partyType") != prev_officer.get("partyType"):
            ...
            if (
                prev_officer.get("firstName").upper() != officer.get("firstName").upper()
                or prev_officer.get("middleName", "").upper() != middle_name.upper()
                or prev_officer.get("lastName").upper() != officer.get("lastName").upper()
            ):
                changed = True
    ...
    def _format_correction_data(self, filing):
        if bool(filing.get("correction", {}).get("commentOnly", False)) or bool(
            filing.get("header", {}).get("correctionBenStatement", False)
        ):
            return
    ...
    def _format_name_request_data(self, filing, versioned_business: Business):
        ...
        if name_request_json:
            ...
            if name_request_json and not (new_legal_name := name_request_json.get("legalName")):
                new_legal_name = Business.generate_numbered_legal_name(
                    name_request_json["legalType"], versioned_business.identifier
                )
    ...
    def _format_name_translations_data(self, filing, prev_completed_filing: Filing):
        ...
        versioned_name_translations = VersionedBusinessDetailsService.get_name_translations_revision(
            prev_completed_filing.transaction_id, self._business.id
        )
    ...
    def _format_share_class_data(self, filing, prev_completed_filing: Filing):  # pylint: disable=too-many-locals; # noqa: E501;
        ...
        if filing.get("shareClasses"):
            prev_share_class_json = VersionedBusinessDetailsService.get_share_class_revision(
                prev_completed_filing.transaction_id, prev_completed_filing.business_id
            )
    ...
    def _format_share_series_data(self, share_class, filing, prev_completed_filing: Filing):  # pylint: disable=too-many-locals; # noqa: E501;
        if share_class.get("series"):
            prev_share_series_json = VersionedBusinessDetailsService.get_share_series_revision(
                prev_completed_filing.transaction_id, share_class.get("id")
            )
...
class ReportMeta:  # pylint: disable=too-few-public-methods
    ...
    reports = {
        ...
        "continuationIn": {"filingDescription": "Continuation Application", "fileName": "continuationApplication"},
        "certificateOfContinuation": {
            "filingDescription": "Certificate of Continuation",
            "fileName": "certificateOfContinuation",
        },
        "intentToLiquidate": {"filingDescription": "Statement of Intent to Liquidate", "fileName": "intentToLiquidate"},
        "noticeOfWithdrawal": {"filingDescription": "Notice of Withdrawal", "fileName": "noticeOfWithdrawal"},
        "appointReceiver": {"filingDescription": "Appoint Receiver", "fileName": "appointReceiver"},
        "ceaseReceiver": {"filingDescription": "Cease Receiver", "fileName": "ceaseReceiver"},
    }
