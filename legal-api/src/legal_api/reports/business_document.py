...
class BusinessDocument:
    ...
    def _set_dates(self, business: dict):  # pylint: disable=too-many-branches
        ...
        if epoch_date := business["business"].get("epochFilingDate"):
            business["business"]["epochFilingDate"] = LegislationDatetime.as_legislation_timezone(
                datetime.fromisoformat(epoch_date)
            ).strftime(OUTPUT_DATE_FORMAT)
        if self._business.restoration_expiry_date:
            business["business"]["restorationExpiryDate"] = LegislationDatetime.format_as_report_string(
        ...
        if registration_datetime_str := business["business"].get("registrationDateTime"):
            business["formatted_registration_date"] = LegislationDatetime.format_as_report_string(
    ...
    def _set_receivers(self, business: dict):
        ...
        receiver_json = [
            party_role.json
            for party_role in self._business.party_roles.all()
        ...
        for receiver in receiver_json:
            ...
            appointment_date = LegislationDatetime.as_legislation_timezone_from_date_str(receiver["appointmentDate"])
    ...
    def _set_business_state_changes(self, business: dict):
        ...
        for filing in Filing.get_conversion_filings_by_conv_types(
            self._business.id, ["dissolution", "continuationOut", "putBackOn", "restoration"]
        ):
            state_filings.append(self._format_state_filing(filing))
        ...
        if amalgamating_businesses := AmalgamatingBusiness.get_all_revision(self._business.id, tombstone):
            for amalgamating_business in amalgamating_businesses:
                ...
                state_filings.append({**state_filing, **amalgamation_json})
    ...
    def _set_business_changes(self, business: dict):
        ...
        for filing in Filing.get_filings_by_types(
            self._business.id, ["alteration", "correction", "changeOfName", "changeOfRegistration", "specialResolution"]
        ):
            ...
            if filing_meta:
                ...
                if filing.filing_type == "alteration":
                    ...
                    if filing_changes.get("fromLegalName") or filing.filing_type == "changeOfName":
                        ...
                        name_change_info = {}
                        name_change_info["fromLegalName"] = filing_meta.get("changeOfName").get(
                            "fromLegalName", "Not Available"
                        )
                        name_change_info["toLegalName"] = filing_meta.get("changeOfName").get(
                            "toLegalName", "Not Available"
                        )
                        name_change_info["filingDateTime"] = filing.filing_date.isoformat()
                        name_changes.append(name_change_info)
        ...
        for filing in Filing.get_conversion_filings_by_conv_types(self._business.id, ["changeOfName"]):
            filing_meta = filing.meta_data
            name_change_info = {}
            name_change_info["fromLegalName"] = filing_meta.get("changeOfName").get("fromLegalName", "Not Available")
            name_change_info["toLegalName"] = filing_meta.get("changeOfName").get("toLegalName", "Not Available")
            name_change_info["filingDateTime"] = filing.filing_date.isoformat()
            name_changes.append(name_change_info)
    ...
    def _format_state_filing(self, filing: Filing) -> dict:
        ...
        if filing_type == "dissolution":
            filing_info["filingName"] = BusinessDocument._get_summary_display_name(
                filing_type, filing_meta["dissolution"]["dissolutionType"], self._business.legal_type, None
            )
            if (
                self._business.legal_type in ["SP", "GP"]
            ...
            filing_info["filingName"] = BusinessDocument._get_summary_display_name(
                filing_type, filing_sub_type, self._business.legal_type, None
            )
            if filing_sub_type in ["limitedRestoration", "limitedRestorationExtension"]:
                ...
                filing_info["limitedRestorationExpiryDate"] = LegislationDatetime.format_as_report_expiry_string_1159(
                    expiry_date
                )
            ...
            filing_info["filingName"] = BusinessDocument._get_summary_display_name(filing_type, None, None, reason)
    ...
    def _set_amalgamation_details(self, business: dict):
        ...
        filings = Filing.get_filings_by_types(
            self._business.id, ["amalgamationApplication"]
        ) or Filing.get_conversion_filings_by_conv_types(self._business.id, ["amalgamationApplication"])
        if filings:
            ...
            if (self._epoch_filing_date and amalgamation_application.effective_date < self._epoch_filing_date) or (
                ...
                amalgamation = Amalgamation.get_revision(
                    amalgamation_application.transaction_id, amalgamation_application.business_id
                )
                amalgamating_businesses = AmalgamatingBusiness.get_revision(
                    amalgamation_application.transaction_id, amalgamation.id
                )
                for amalgamating_business in amalgamating_businesses:
    ...
    def _set_continuation_in_details(self, business: dict):
        ...
        continuation_in_filing = Filing.get_filings_by_types(
            self._business.id, ["continuationIn"]
        ) or Filing.get_conversion_filings_by_conv_types(self._business.id, ["continuationIn"])
    ...
    def _get_summary_display_name(
        filing_type: str, filing_sub_type: Optional[str], legal_type: Optional[str], reason: Optional[str]
    ) -> str:
        if filing_type == "dissolution":
    ...
    FILING_SUMMARY_DISPLAY_NAME = {
        ...
        "putBackOff": {"Limited Restoration Expired": "Dissolved due to expired Limited Restoration"},
