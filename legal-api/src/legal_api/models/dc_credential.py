...
class DCCredential(db.Model):  # pylint: disable=too-many-instance-attributes
    ...
    connection_id = db.Column("connection_id", db.Integer, db.ForeignKey("dc_connections.id"), nullable=False)
    credential_exchange_id = db.Column("credential_exchange_id", db.String(100))
    definition_id = db.Column("definition_id", db.Integer, db.ForeignKey("dc_definitions.id"), nullable=False)
    ...
    date_of_revocation = db.Column("date_of_revocation", db.DateTime(timezone=True))

    business_user_id = db.Column("business_user_id", db.Integer, db.ForeignKey("dc_business_users.id"), nullable=False)

    # relationships
    connection = db.relationship("DCConnection", backref="credentials", foreign_keys=[connection_id])
    definition = db.relationship("DCDefinition", backref="credentials", foreign_keys=[definition_id])
    ...
    def find_by_id(cls, credential_id: str) -> DCCredential:
        ...
        dc_credential = None
        if credential_id:
            dc_credential = cls.query.filter_by(id=credential_id).one_or_none()
        return dc_credential
    ...
    def find_by_credential_exchange_id(cls, credential_exchange_id: str) -> DCCredential:
        ...
        if credential_exchange_id:
            dc_credential = cls.query.filter(
                DCCredential.credential_exchange_id == credential_exchange_id
            ).one_or_none()
        return dc_credential
    ...
    def find_by_credential_id(cls, credential_id: str) -> DCCredential:
        ...
        dc_credential = None
        if credential_id:
            dc_credential = cls.query.filter(DCCredential.credential_id == credential_id).one_or_none()
        return dc_credential
    ...
    def find_by_connection_id(cls, connection_id: str) -> DCCredential:
        ...
        dc_credential = None
        if connection_id:
            dc_credential = cls.query.filter(DCCredential.connection_id == connection_id).one_or_none()
        return dc_credential
