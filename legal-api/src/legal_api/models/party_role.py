...
class PartyRole(Versioned, db.Model):
    ...
    @staticmethod
    def get_party_roles(legal_entity_id: int, end_date: datetime = None, role: str = None) -> list:
        """Return the parties that match the filter conditions."""
        party_roles = db.session.query(PartyRole).filter(PartyRole.legal_entity_id == legal_entity_id)
