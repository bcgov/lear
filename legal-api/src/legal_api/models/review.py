...
class Review(db.Model):  # pylint: disable=too-many-instance-attributes
    ...
    contact_details = db.Column('contact_details', db.String(150))
    ...
    def get_paginated_reviews(cls, review_filter, mapped_sort_by_column):
        ...
        if review_filter.contact_details:
            query = query.filter(Review.contact_details.ilike(f'%{review_filter.contact_details}%'))
    ...
    def json(self) -> dict:
        ...
        return {
            ...
            'officeEmail': self.contact_details,
