...
class Review(db.Model):  # pylint: disable=too-many-instance-attributes
    ...
    submission_date = db.Column(
        "submission_date", db.DateTime(timezone=True), default=datetime.utcnow
    ...
    def get_review(cls, filing_id) -> Review:
        ...
        review = None
        if filing_id:
            review = db.session.query(Review).filter(Review.filing_id == filing_id).one_or_none()
        return review
    ...
    def get_paginated_reviews(cls, review_filter, mapped_sort_by_column):
        ...
        if review_filter.start_date:
            start_date = datetime.combine(
                LegislationDatetime.as_utc_timezone_from_legislation_date_str(review_filter.start_date),
                datetime.min.time(),
            )
            query = query.filter(Review.submission_date >= start_date)
        if review_filter.end_date:
            end_date = datetime.combine(
                LegislationDatetime.as_utc_timezone_from_legislation_date_str(review_filter.end_date),
                datetime.max.time(),
            )
        ...
        reviews_list = [review.json for review in results]
