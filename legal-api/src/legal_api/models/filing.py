...
class Filing(db.Model):  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    FILINGS = {
        ...
        "continuationIn": {
            ...
            "codes": {"C": "CONTI", "CBEN": "CONTI", "CUL": "CONTI", "CCC": "CONTI"},
        },
        "continuationOut": {
    ...
    # # relationships
    transaction_id = db.Column("transaction_id", db.BigInteger, db.ForeignKey("transaction.id"))
    ...
    parent_filing = db.relationship(
        "Filing", remote_side=[id], backref=backref("children", uselist=True), foreign_keys=[parent_filing_id]
    )

    withdrawn_filing_id = db.Column("withdrawn_filing_id", db.Integer, db.ForeignKey("filings.id"))
    withdrawn_filing = db.relationship("Filing", remote_side=[id], foreign_keys=[withdrawn_filing_id])
    ...
    def set_processed(self):
        ...
        if not self.effective_date_can_be_before_payment_completion_date() and (
            ...
            or (
                self.payment_completion_date and self.effective_date < self.payment_completion_date  # pylint: disable=comparison-with-callable
            )
        ):
    ...
    def comments_count(self):
        ...
        return select([func.count(Comment.business_id)]).where(Comment.business_id == self.id).label("comments_count")
    ...
    def get_temp_reg_filing(temp_reg_id: str, filing_id: str = None):
        ...
        return (
            ...
            .filter(
                db.or_(
                    db.and_(Filing.id == filing_id, Filing.temp_reg == temp_reg_id),
                    db.and_(  # special case for NoW
                        Filing.id == filing_id,
                        Filing._filing_type == "noticeOfWithdrawal",
                        Filing.withdrawn_filing_id
    ...
    def get_conversion_filings_by_conv_types(business_id: int, filing_types: list):
        ...
        filings = (
            ...
            .filter(Filing._meta_data.op("->")("conversion").op("->>")("convFilingType").in_(filing_types))
            .order_by(desc(Filing.transaction_id))
            .all()
        )
    ...
    def get_completed_filings_for_colin(limit=20, offset=0):
        ...
        filings = (
            ...
            .order_by(Filing.transaction_id)
            .limit(limit)
            .offset(offset)
            .all()
        )
    ...
    def get_all_filings_by_status(status):
        ...
        filings = db.session.query(Filing).filter(Filing._status == status).all()  # pylint: disable=singleton-comparison # noqa: E711;
        return filings
    ...
    def set_review_decision(self, filing_status):
        ...
        if filing_status not in [
            ...
            raise BusinessException(
                error=f"Cannot set this filing status {filing_status}.", status_code=HTTPStatus.FORBIDDEN
            )
    ...
    def submit_filing_to_awaiting_review(self, submission_date):
        ...
        if self._status not in [Filing.Status.DRAFT.value, Filing.Status.CHANGE_REQUESTED.value]:
            raise BusinessException(
                error="Cannot submit this filing to awaiting review status.", status_code=HTTPStatus.FORBIDDEN
            )
    ...
    def legal_filings(self) -> List:
        ...
        for k in filing["filing"].keys():  # pylint: disable=unsubscriptable-object
            if Filing.FILINGS.get(k, None):
                legal_filings.append({k: copy.deepcopy(filing["filing"].get(k))})  # pylint: disable=unsubscriptable-object
...
def receive_before_change(mapper, connection, target):  # pylint: disable=unused-argument; SQLAlchemy callback signature
    ...
    if filing._status in [
        Filing.Status.AWAITING_REVIEW.value,
        Filing.Status.CHANGE_REQUESTED.value,
        Filing.Status.REJECTED.value,
        Filing.Status.WITHDRAWN.value,
    ] or (filing._status == Filing.Status.APPROVED.value and not filing.payment_token):
