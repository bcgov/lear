...
class ReviewResult(db.Model):  # pylint: disable=too-many-instance-attributes
    ...
    reviewer = db.relationship("User", backref=backref("reviewer", uselist=False), foreign_keys=[reviewer_id])
    ...
    def get_review_results(cls, review_id) -> List[ReviewResult]:
        ...
        review_results = None
        if review_id:
            review_results = db.session.query(ReviewResult).filter(ReviewResult.review_id == review_id).all()
