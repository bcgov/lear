...
def validate(comment: Dict, is_filing: bool) -> Error:
    ...
    if is_filing:
        ...
        error = PermissionService.check_user_permission(
            ListActionsPermissionsAllowed.DETAIL_COMMENTS.value, message=message
        )
        if error:
        ...
        error = PermissionService.check_user_permission(
            ListActionsPermissionsAllowed.STAFF_COMMENTS.value, message=message
        )
        if error:
