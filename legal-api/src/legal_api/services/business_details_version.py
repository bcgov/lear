...
class VersionedBusinessDetailsService:  # pylint: disable=too-many-public-methods
    ...
    def get_ia_revision(filing, business) -> dict:
        ...
        ia_json["incorporationApplication"]["offices"] = VersionedBusinessDetailsService.get_office_revision(
            filing.id, filing.transaction_id, business.id
        )
    ...
    def get_company_details_revision(filing_id, business_id) -> dict:
        ...
        company_profile_json["parties"] = VersionedBusinessDetailsService.get_party_role_revision(filing, business_id)
        company_profile_json["offices"] = VersionedBusinessDetailsService.get_office_revision(
            filing_id, filing.transaction_id, business_id
        )
        company_profile_json["shareClasses"] = VersionedBusinessDetailsService.get_share_class_revision(
            filing.transaction_id, business_id
        )
    ...
    def get_business_revision(filing, business) -> dict:
        ...
        business_revision = (
            ...
            .filter(
                or_(
                    business_version.end_transaction_id == None,  # pylint: disable=singleton-comparison # noqa: E711,E501;
                    business_version.end_transaction_id > filing.transaction_id,
                )
            )
            .order_by(business_version.transaction_id)
            .one_or_none()
        )
    ...
    def find_last_value_from_business_revision(
        filing, is_dissolution_date=False, is_restoration_expiry_date=False
    ) -> dict:
    ...
    def get_office_revision(filing_id, transaction_id, business_id) -> dict:
        ...
        versioned_offices = (
            db.session.query(offices_version)
            ...
            .filter(
                or_(
                    offices_version.end_transaction_id == None,  # pylint: disable=singleton-comparison # noqa: E711,E501;
                    offices_version.end_transaction_id > transaction_id,
                )
            )
            .order_by(offices_version.transaction_id)
            .all()
        )
        ...
        for office in versioned_offices:
            ...
            addresses_list = (
                ...
                .filter(
                    or_(
                        address_version.end_transaction_id == None,  # pylint: disable=singleton-comparison # noqa: E711,E501;
                        address_version.end_transaction_id > transaction_id,
                    )
                )
                .order_by(address_version.transaction_id)
                .all()
            )
    ...
    def get_party_role_revision(filing, business_id, is_ia_or_after=False, role=None) -> dict:
        ...
        versioned_party_roles = (
            ...
            .filter(
                or_(
                    party_role_version.end_transaction_id == None,  # pylint: disable=singleton-comparison # noqa: E711,E501;
                    party_role_version.end_transaction_id > filing.transaction_id,
                )
            )
            .filter(
                party_role_version.role.notin_(
                    [
            ...
            .order_by(party_role_version.transaction_id)
            .all()
        )
    ...
    def get_share_class_revision(transaction_id, business_id) -> dict:
        ...
        share_classes_list = (
            ...
            .filter(
                or_(
                    share_class_version.end_transaction_id == None,  # pylint: disable=singleton-comparison # noqa: E711,E501;
                    share_class_version.end_transaction_id > transaction_id,
                )
            )
            .order_by(share_class_version.transaction_id)
            .all()
        )
        share_classes = []
    ...
    def get_share_series_revision(transaction_id, share_class_id) -> dict:
        ...
        share_series_list = (
            ...
            .filter(
                or_(
                    share_series_version.end_transaction_id == None,  # pylint: disable=singleton-comparison # noqa: E711,E501;
                    share_series_version.end_transaction_id > transaction_id,
                )
            )
            .order_by(share_series_version.transaction_id)
            .all()
        )
        share_series_arr = []
    ...
    def get_name_translations_revision(transaction_id, business_id) -> dict:
        ...
        name_translations_list = (
            ...
            .filter(
                or_(
                    name_translations_version.end_transaction_id == None,  # pylint: disable=singleton-comparison # noqa: E711,E501;
                    name_translations_version.end_transaction_id > transaction_id,
                )
            )
            .order_by(name_translations_version.transaction_id)
            .all()
        )
    ...
    def get_resolution_dates_revision(transaction_id, business_id) -> dict:
        ...
        resolution_list = (
            ...
            .filter(
                or_(
                    resolution_version.end_transaction_id == None,  # pylint: disable=singleton-comparison # noqa: E711,E501;
    ...
    def party_role_revision_json(filing, party_role, is_ia_or_after) -> dict:
        ...
        party = VersionedBusinessDetailsService.party_revision_json(
            filing.transaction_id, party_revision, is_ia_or_after
        )
    ...
    def get_party_revision(filing, party_id) -> dict:
        ...
        party = (
            ...
            .filter(
                or_(
                    party_version.end_transaction_id == None,  # pylint: disable=singleton-comparison # noqa: E711,E501;
                    party_version.end_transaction_id > filing.transaction_id,
                )
            )
            .order_by(party_version.transaction_id)
            .one_or_none()
        )
        ...
        if isinstance(party, VersioningProxy.version_class(db.session(), Party)):
            if party.mailing_address_id:
                member_mailing_address = VersionedBusinessDetailsService.address_revision_json(
                    VersionedBusinessDetailsService.get_address_revision(transaction_id, party.mailing_address_id)
                )
