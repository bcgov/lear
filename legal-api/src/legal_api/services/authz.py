...
def authorized(  # pylint: disable=too-many-return-statements
    ...
    if (
        jwt.validate_roles([MAXIMUS_STAFF_ROLE])
        or jwt.validate_roles([SBC_STAFF_ROLE])
        or jwt.validate_roles([CONTACT_CENTRE_STAFF_ROLE])
    ):
        return True
    ...
    return {
        "staff": {
            Business.State.ACTIVE: {
                ...
                "courtOrder": {"legalTypes": ["SP", "GP", "CP", "BC", "BEN", "CC", "ULC", "C", "CBEN", "CUL", "CCC"]},
                "dissolution": {
                    "voluntary": {
                ...
                "putBackOff": {"legalTypes": ["BEN", "BC", "CC", "ULC", "C", "CBEN", "CUL", "CCC"]},
                "registrarsNotation": {
                    "legalTypes": ["SP", "GP", "CP", "BC", "BEN", "CC", "ULC", "C", "CBEN", "CUL", "CCC"]
                },
                "registrarsOrder": {
                ...
                "noticeOfWithdrawal": {
                    "legalTypes": ["BC", "BEN", "CC", "ULC", "C", "CBEN", "CUL", "CCC"],
                    "blockerChecks": {"business": [BusinessBlocker.FILING_WITHDRAWAL]},
                    "businessRequirement": BusinessRequirement.NO_RESTRICTION,
                },
                "intentToLiquidate": {
                    "legalTypes": ["BC", "BEN", "CC", "ULC", "C", "CBEN", "CUL", "CCC"],
            ...
            Business.State.HISTORICAL: {
                ...
                "restoration": {
                    "fullRestoration": {
                        "legalTypes": ["BC", "BEN", "CC", "ULC", "C", "CBEN", "CUL", "CCC"],
                        "blockerChecks": {
                            "invalidStateFilings": ["amalgamationOut", "continuationOut"],
...
def get_could_files(jwt: JwtManager, business_type: str, business_state: str):
    ...
    result = {"filing": {"filingTypes": allowed_filings}}
    return result
...
def get_could_file(legal_type: str, state: str, jwt: JwtManager):
    ...
    for allowable_filing_key, allowable_filing_value in allowable_filings.items():
        ...
        if allowable_filing_legal_types:
            ...
            could_filing_types = add_allowable_filing_type(is_allowable, could_filing_types, allowable_filing_type)
            continue
        ...
        for filing_sub_type_item_key, _ in filing_sub_type_items:
            ...
            could_filing_types = add_allowable_filing_type(True, could_filing_types, allowable_filing_sub_type)

    return could_filing_types


def get_allowed_filings(
    business: Business, state: Business.State, legal_type: str, jwt: JwtManager, is_ignore_draft_blockers: bool = False
):
    """Get allowed type of filing types for the current user."""
    # importing here to avoid circular dependencies
    # pylint: disable=import-outside-toplevel
...
def has_blocker_completed_filing(business: Business, blocker_checks: dict):
    ...
    completed_filings = Filing.get_filings_by_type_pairs(
        business.id, filing_type_pairs, [Filing.Status.COMPLETED.value], True
    )
...
def has_blocker_future_effective_filing(business: Business, blocker_checks: dict):
    ...
    pending_filings = Filing.get_filings_by_type_pairs(
        business.id, filing_type_pairs, [Filing.Status.PENDING.value, Filing.Status.PAID.value], True
    )
...
def has_notice_of_withdrawal_filing_blocker(business: Business, is_ignore_draft_blockers: bool = False):
    ...
    filing_statuses = [Filing.Status.PENDING.value, Filing.Status.PENDING_CORRECTION.value, Filing.Status.ERROR.value]
    if not is_ignore_draft_blockers:
