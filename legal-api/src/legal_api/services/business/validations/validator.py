...
document_rule_set = {
    "cstat": {"excluded_types": ["SP", "GP"], "status": Business.State.ACTIVE},
    "cogs": {"excluded_types": ["SP", "GP"], "goodStanding": True},
    "lseal": {
...
def validate_document_request(document_type, business: Business):
    ...
    if document_type == "summary" and business.legal_type not in enabled_filings:
        return Error(
            HTTPStatus.FORBIDDEN, [{"error": babel(f"{business.legal_type} is not enabled business summary.")}]
        )
