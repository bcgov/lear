...
class DigitalCredentialsService:
    ...
    def _fetch_credential_definition(self, cred_def_id: str) -> Optional[str]:
        ...
        try:
            response = requests.get(
                self.api_url + f"/credential-definitions/{cred_def_id}", headers=self._get_headers()
            )
            response.raise_for_status()
    ...
    def attest_connection(self, connection_id: str) -> Optional[dict]:
        ...
        try:
            ...
            response = requests.post(
                self.api_url + "/present-proof-2.0/send-request",
                headers=self._get_headers(),
                data=json.dumps(
                    {
                            ...
                            "indy": {
                                ...
                                "requested_attributes": {
                                    "attestationInfo": {
                                        ...
                                        "restrictions": [{"cred_def_id": self.wallet_cred_def_id}],
                                    }
                                },
