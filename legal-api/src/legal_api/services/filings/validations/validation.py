...
def validate(
    business: Business,  # pylint: disable=too-many-branches,too-many-statements
    filing_json: Dict,
    account_id=None,
) -> Error:
    ...
    if "correction" in filing_json["filing"].keys():
        ...
        if (
            ...
            and dissolution_type != DissolutionTypes.ADMINISTRATIVE
        ):
        ...
        if err:
            return err
    elif "specialResolution" in filing_json["filing"].keys() and business.legal_type in [
