...
def validate_offices(filing_json: dict, legal_type: str, filing_type: str = "incorporationApplication") -> list:
    ...
    for item in addresses.keys():
        ...
        if legal_type in Business.CORPS and "recordsOffice" not in addresses.keys():
            msg.append({"error": "recordsOffice is required", "path": f"/filing/{filing_type}/offices"})
...
def validate_coop_parties_mailing_address(
    incorporation_json: dict, filing_type: str = "incorporationApplication"
) -> list:
...
def validate_parties_delivery_address(
    ...
    for idx, party in enumerate(parties_array):
        ...
        if is_director:
            if "deliveryAddress" not in party:
                msg.append(
                    {"error": babel("deliveryAddress is required."), "path": f"/filing/{filing_type}/parties/{idx}"}
                )
                continue
