...
def validate_offices(filing_json: dict, filing_type: str = "incorporationApplication") -> list:
    ...
    for item in addresses.keys():
        if item in ("registeredOffice", "recordsOffice"):
            ...
            msg.append(
                {
                    "error": f"Invalid office {item}. Only registeredOffice and recordsOffice are allowed.",
...
def _validate_address(addresses: dict, address_key: str, filing_type: str) -> list:
    ...
    for k, v in addresses[address_key].items():
        ...
        if region != "BC":
            path = f"/filing/{filing_type}/offices/%s/%s/addressRegion" % (address_key, k)
            msg.append({"error": "Address Region must be 'BC'.", "path": path})
        ...
        try:
            ...
            err_path = f"/filing/{filing_type}/offices/%s/%s/addressCountry" % (address_key, k)
            msg.append({"error": "Address Country must be 'CA'.", "path": err_path})
...
def validate_roles(filing_dict: dict, entity_type: str, filing_type: str = "incorporationApplication") -> Error:
    ...
    if filing_type == "incorporationApplication" or (
        ...
        and filing_dict["filing"][filing_type].get("type") == "STAFF"
        and completing_party_count != 0
    ):
        err_path = f"/filing/{filing_type}/parties/roles"
...
def validate_parties_mailing_address(
    ...
    for item in parties_array:
        for k, v in item["mailingAddress"].items():
            if v is None:
                err_path = f"/filing/{filing_type}/parties/%s/mailingAddress/%s/%s/" % (item["officer"]["id"], k, v)
                msg.append(
                    {
                        "error": "Person %s: Mailing address %s %s is invalid" % (item["officer"]["id"], k, v),
                        "path": err_path,
                    }
                )
...
def validate_correction_ia(filing: dict) -> Optional[Error]:
    ...
    if not (
        corrected_filing := Filing.find_by_id(  # pylint: disable=superfluous-parens; needed to pass pylance
            filing["filing"]["correction"]["correctedFilingId"]
        )
    ):
        return Error(HTTPStatus.BAD_REQUEST, [{"error": babel("Missing the id of the filing being corrected.")}])
