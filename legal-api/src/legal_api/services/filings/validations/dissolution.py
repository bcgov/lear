...
def validate_dissolution_type(filing_json, legal_type) -> Optional[list]:
    ...
    if dissolution_type:
        ...
        if (
            (legal_type == Business.LegalTypes.COOP.value and dissolution_type not in DISSOLUTION_MAPPING["COOP"])
            or (legal_type in Business.CORPS and dissolution_type not in DISSOLUTION_MAPPING["CORP"])
            or (
                legal_type in (Business.LegalTypes.SOLE_PROP.value, Business.LegalTypes.PARTNERSHIP.value)
        ...
        msg.append({"error": _("Dissolution type must be provided."), "path": dissolution_type_path})
        return msg
...
def validate_dissolution_statement_type(filing_json, legal_type, dissolution_type) -> Optional[list]:
    ...
    if legal_type == Business.LegalTypes.COOP.value:
        ...
        if not DissolutionStatementTypes.has_value(dissolution_stmt_type):
            msg.append({"error": _("Invalid Dissolution statement type."), "path": dissolution_stmt_type_path})
            return msg
...
def _is_dissolution_party_role(roles: list) -> bool:
    return any(
        role.get("roleType", "").lower() in [PartyRole.RoleTypes.CUSTODIAN.value, PartyRole.RoleTypes.LIQUIDATOR.value]
        for role in roles
    )
    ...
    for idx, party in enumerate(parties):  # pylint: disable=too-many-nested-blocks;  # noqa: E501
        for address_type in Address.JSON_ADDRESS_TYPES:
            if address_type in party:
                ...
                msg.append({"error": _(f"{address_type} is required."), "path": f"/filing/dissolution/parties/{idx}"})
...
def validate_affidavit(filing_json, legal_type, dissolution_type) -> Optional[list]:
    ...
    if legal_type == Business.LegalTypes.COOP.value:
        ...
        if not affidavit_file_key:
            return [{"error": _("A valid affidavit key is required."), "path": affidavit_file_key_path}]
...
def _validate_custodian_email(parties, dissolution_type, legal_type) -> list:
    ...
    for idx, party in enumerate(parties):
        ...
        if not email:
            msg.append(
                {
                    "error": "Custodian email is required for voluntary dissolution.",
...
def validate_custodial_office(filing_json, legal_type, dissolution_type) -> Optional[list]:
    ...
    if "custodialOffice" not in filing_json["filing"]["dissolution"]:
        return [
            {
                "error": "Custodial office is required for voluntary dissolution.",
