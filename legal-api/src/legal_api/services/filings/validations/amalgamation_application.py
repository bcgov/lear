...
def validate(amalgamation_json: Dict, account_id) -> Optional[Error]:
    ...
    msg.extend(
        validate_amalgamating_businesses(amalgamation_json, filing_type, legal_type, amalgamation_type, account_id)
    )
...
def validate_amalgamating_businesses(  # pylint: disable=too-many-branches,too-many-statements,too-many-locals
    ...
    if legal_type not in enabled_filings:
        return Error(
            HTTPStatus.FORBIDDEN, [{"error": babel(f"{legal_type} is not enabled for amalgamation entities.")}]
        )
    msg = []
    ...
    is_any_bc_company = (
        is_any_business[Business.LegalTypes.BCOMP.value]
        or is_any_business[Business.LegalTypes.COMP.value]
        or is_any_business[Business.LegalTypes.BC_CCC.value]
        or is_any_business[Business.LegalTypes.BC_ULC_COMPANY.value]
    )
    ...
    if primary_or_holding_business:
        ...
        legal_type_to_compare = continued_types_map.get(
            primary_or_holding_business.legal_type, primary_or_holding_business.legal_type
        )
    ...
    if legal_type == Business.LegalTypes.BC_CCC.value and not is_any_business[Business.LegalTypes.BC_CCC.value]:
        ...
        legal_type in [Business.LegalTypes.BC_CCC.value, Business.LegalTypes.BC_ULC_COMPANY.value]
        and is_any_expro_a
