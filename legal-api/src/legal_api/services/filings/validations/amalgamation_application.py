...
def validate_amalgamating_businesses(  # pylint: disable=too-many-branches,too-many-statements,too-many-locals
    ...
    is_any_business = {
        ...
        LegalEntity.EntityTypes.BC_CCC.value: False,
        LegalEntity.EntityTypes.BC_ULC_COMPANY.value: False,
    }
    ...
    is_any_bc_company = (
        is_any_business[LegalEntity.EntityTypes.BCOMP.value]
        or is_any_business[LegalEntity.EntityTypes.COMP.value]
        or is_any_business[LegalEntity.EntityTypes.BC_CCC.value]
        or is_any_business[LegalEntity.EntityTypes.BC_ULC_COMPANY.value]
    )
        ...
        if foreign_legal_name := amalgamating_business_json.get("legalName"):
            msg.extend(
                _validate_foreign_businesses(
                    is_staff,
                    is_any_bc_company,
                    is_any_business[LegalEntity.EntityTypes.BC_ULC_COMPANY.value],
                    legal_type,
                    foreign_legal_name,
            ...
            msg.extend(
                _validate_lear_businesses(
                    identifier, amalgamating_business, account_id, is_staff, amalgamating_businesses_path
                )
            )
    ...
    if legal_type == LegalEntity.EntityTypes.BC_CCC.value and not is_any_business[LegalEntity.EntityTypes.BC_CCC.value]:
        msg.append(
            {
                "error": (
...
def _validate_foreign_businesses(  # pylint: disable=too-many-arguments
    is_staff, is_any_bc_company, is_any_ulc, legal_type, foreign_legal_name, role, amalgamating_businesses_path
) -> list:
    msg = []
    if is_staff:
        ...
        if role in [AmalgamatingBusiness.Role.primary.name, AmalgamatingBusiness.Role.holding.name]:
            msg.append(
                {
...
def _validate_lear_businesses(  # pylint: disable=too-many-arguments
    ...
    msg = []
    if isinstance(amalgamating_business, LegalEntity):
        ...
        if not is_staff:
            ...
            if not amalgamating_business.good_standing:
                msg.append({"error": f"{identifier} is not in good standing.", "path": amalgamating_businesses_path})
    else:
        msg.append(
            {"error": f"A business with identifier:{identifier} not found.", "path": amalgamating_businesses_path}
        )
