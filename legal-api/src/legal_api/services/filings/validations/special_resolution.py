...
def validate(business: any, filing_json: Dict) -> Error:
...
def validate_resolution_content(filing_json, filing_type: str = "specialResolution"):
    ...
    if not resolution_content:
        msg.append({"error": _("Resolution must be provided."), "path": resolution_path})
        return msg

    if len(resolution_content) > 2097152:
        msg.append({"error": _("Resolution must be 2MB or less."), "path": resolution_path})
    return msg


def validate_resolution_date(business: any, filing_json: Dict) -> Optional[list]:
    ...
    if resolution_date < business.founding_date.date():
        msg.append(
            {"error": _("Resolution date cannot be earlier than the incorporation date."), "path": resolution_date_path}
        )
        return msg

    if resolution_date > datetime.utcnow().date():
        msg.append({"error": _("Resolution date cannot be in the future."), "path": resolution_date_path})
        return msg
...
def validate_signing_date(filing_json: Dict, filing_type: str = "specialResolution") -> Optional[list]:
    ...
    if signing_date > datetime.utcnow().date():
        msg.append({"error": _("Signing date cannot be in the future."), "path": signing_date_path})
        return msg
    ...
    if signing_date < resolution_date:
        msg.append({"error": _("Signing date cannot be before the resolution date."), "path": signing_date_path})
        return msg
