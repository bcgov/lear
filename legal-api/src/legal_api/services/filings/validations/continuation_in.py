...
def validate(filing_json: dict) -> Optional[Error]:  # pylint: disable=too-many-branches;
    ...
    if legal_type not in enabled_filings:
        return Error(HTTPStatus.FORBIDDEN, [{"error": babel(f"{legal_type} does not support continuation in filing.")}])
...
def _validate_foreign_jurisdiction(filing_json: dict, filing_type: str, legal_type: str) -> list:
    ...
    if err := validate_foreign_jurisdiction(foreign_jurisdiction, foreign_jurisdiction_path):
        ...
        legal_type == Business.LegalTypes.ULC_CONTINUE_IN.value
        and foreign_jurisdiction["country"] == "CA"
        and ((region := foreign_jurisdiction.get("region")) and region == "AB")
    ):
    ...
    try:
        ...
        if incorporation_date_formatted > dt.now():
            msg.append({"error": "Incorporation date cannot be in the future.", "path": incorporation_date_path})
    except ValueError:
        msg.append(
            {
...
def validate_business_in_colin(filing_json: dict, filing_type: str) -> list:
    ...
    if filing_json["filing"][filing_type].get("business"):
        ...
        if response.status_code != HTTPStatus.OK:
            msg.append(
                {"error": "Could not fetch business data for company from Colin.", "path": business_identifier_path}
            )
        elif legal_name != response_json["business"]["legalName"]:
            msg.append(
                {
                    "error": "Legal name does not match with company legal name from Colin.",
