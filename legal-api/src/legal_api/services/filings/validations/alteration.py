...
def company_name_validation(filing, business: Business):
    ...
    if get_str(filing, "/filing/alteration/nameRequest/nrNumber"):
        accepted_request_types = [
            ...
            "BECC",
            "ULCB",
            "ULBE",  # conversion types
        ]
        msg.extend(
            validate_name_request(filing, new_legal_type or business.legal_type, "alteration", accepted_request_types)
        )
    else:
...
def type_change_validation(filing, business: Business):
    ...
    errors = {
        ...
        Business.LegalTypes.BCOMP.value: (
            """BC Benefit Company can only change to BC Limited Company or
                                          BC Community Contribution Company."""
        ),
        Business.LegalTypes.COMP.value: (
            """BC Limited Company can only change to BC Benefit Company or
        ...
        Business.LegalTypes.BC_ULC_COMPANY.value: (
            """BC Unlimited Liability Company can only change to
