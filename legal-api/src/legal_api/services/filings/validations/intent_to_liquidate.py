    ...
    for address_type in ["mailingAddress", "deliveryAddress"]:
        ...
        if address:
            ...
            if region and region != "BC":
                msg.append({"error": babel("Address Region must be 'BC'."), "path": f"{address_path}/addressRegion"})

            country = address.get("addressCountry")
            if country and country != "CA":
                msg.append({"error": babel("Address Country must be 'CA'."), "path": f"{address_path}/addressCountry"})
