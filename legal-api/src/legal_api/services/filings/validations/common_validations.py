...
def has_at_least_one_share_class(filing_json, filing_type) -> Optional[str]:  # pylint: disable=too-many-branches
    ...
    if filing_type in filing_json["filing"] and "shareStructure" in filing_json["filing"][filing_type]:
        share_classes = filing_json["filing"][filing_type].get("shareStructure", {}).get("shareClasses", [])
...
def validate_resolution_date_in_share_structure(filing_json, filing_type) -> Optional[dict]:
    ...
    if any(x.get("hasRightsOrRestrictions", False) for x in share_classes) or any(
        has_rights_or_restrictions_true_in_share_series(x) for x in share_classes
    ):
...
def validate_share_structure(incorporation_json, filing_type, legal_type) -> Error:  # pylint: disable=too-many-branches
    ...
    share_classes = incorporation_json["filing"][filing_type].get("shareStructure", {}).get("shareClasses", [])
    msg = []
    memoize_names = []
...
def validate_series(item, memoize_names, filing_type, index) -> Error:
    ...
    for series_index, series in enumerate(item.get("series", [])):
        ...
        if not stripped_series_name:
            msg.append({"error": "Share series name is required.", "path": f"{err_path}/name/"})

        elif series_name != stripped_series_name:
            msg.append({"error": "Share series name cannot start or end with whitespace.", "path": f"{err_path}/name/"})

        elif series_name in memoize_names:
            msg.append(
                {
                    "error": "Share series %s name already used in a share class or series." % series_name,
                    "path": err_path,
                }
            )
        else:
        ...
        if series["hasMaximumShares"]:
            if not series.get("maxNumberOfShares", None):
                ...
                if (
                    ...
                    and int(series["maxNumberOfShares"]) > int(item["maxNumberOfShares"])
                ):
                    msg.append(
                        {
...
def validate_shares(item, memoize_names, filing_type, index, legal_type) -> Error:
    ...
    if not stripped_share_name:
        ...
        msg.append({"error": "Share class name is required.", "path": err_path})

    elif share_name != stripped_share_name:
        err_path = "/filing/{0}/shareClasses/{1}/name/".format(filing_type, index)
        msg.append({"error": "Share class name cannot start or end with whitespace.", "path": err_path})

    elif share_name in memoize_names:
        err_path = "/filing/{0}/shareClasses/{1}/name/".format(filing_type, index)
        msg.append(
            {"error": "Share class %s name already used in a share class or series." % share_name, "path": err_path}
        )
    else:
    ...
    if item["hasMaximumShares"] and not item.get("maxNumberOfShares", None):
        ...
        msg.append(
            {"error": "Share class %s must provide value for maximum number of shares" % item["name"], "path": err_path}
        )
    if item["hasParValue"]:
...
def validate_court_order(court_order_path, court_order):
    ...
    if "fileNumber" not in court_order:
        ...
        if len(court_order["fileNumber"]) < 5 or len(court_order["fileNumber"]) > 20:
            err_path = court_order_path + "/fileNumber"
            msg.append(
                {"error": "Length of court order file number must be from 5 to 20 characters.", "path": err_path}
            )
    ...
    try:
        ...
        if verify_paper_size:
            ...
            if any(x.mediaBox.getWidth() != 612 or x.mediaBox.getHeight() != 792 for x in pdf_reader.pages):
                msg.append(
                    {
                        "error": _("Document must be set to fit onto 8.5” x 11” letter-size paper."),
                        "path": file_key_path,
                    }
                )
...
def validate_party_name(party: dict, party_path: str, legal_type: str) -> list:
    ...
    if party_type == "person":
        ...
        if (legal_type in Business.CORPS) and (not stripped_first_name):
            ...
            msg.append(
                {"error": f"{party_roles_str} first name cannot start or end with whitespace", "path": party_path}
            )
        ...
        if middle_initial is not None and middle_initial.strip():
            if middle_initial != middle_initial.strip():
                msg.append(
                    {
                        "error": f"{party_roles_str} middle initial cannot start or end with whitespace",
                        "path": party_path,
                    }
                )
            elif len(middle_initial) > custom_allowed_max_length:
                err_msg = (
                    f"{party_roles_str} middle initial cannot be longer than {custom_allowed_max_length} characters"
                )
                msg.append({"error": err_msg, "path": party_path})
        ...
        if middle_name is not None and middle_name.strip():
            if middle_name != middle_name.strip():
                msg.append(
                    {"error": f"{party_roles_str} middle name cannot start or end with whitespace", "path": party_path}
                )
        ...
        if (legal_type in Business.CORPS) and (not stripped_last_name):
            ...
            msg.append(
                {"error": f"{party_roles_str} last name cannot start or end with whitespace", "path": party_path}
            )
...
def validate_name_request(
    filing_json: dict,  # pylint: disable=too-many-locals
    legal_type: str,
    filing_type: str,
    ...
    if not nr_number and not legal_name:
        ...
        return [{"error": _("nrNumber is missing for the legal name provided in nameRequest."), "path": nr_number_path}]
    ...
    if accepted_request_types and nr_response_json["requestTypeCd"] not in accepted_request_types:
        msg.append(
            {
                "error": _("The name type associated with the name request number entered cannot be used."),
                "path": nr_number_path,
            }
        )
    ...
    if nr_name != legal_name:
        msg.append(
            {"error": _("Name Request legal name is not same as the business legal name."), "path": legal_name_path}
        )

    return msg


def validate_foreign_jurisdiction(
    foreign_jurisdiction: dict, foreign_jurisdiction_path: str, is_region_bc_valid=False, is_region_for_us_required=True
) -> list:
    ...
    if not country:
        ...
        country_code == "US"
        and is_region_for_us_required
        and not pycountry.subdivisions.get(code=f"{country_code}-{region}")
    ):
        msg.append({"error": "Invalid region.", "path": f"{foreign_jurisdiction_path}/region"})
...
def validate_addresses(addresses: dict, addresses_path: str) -> list:
...
def _validate_postal_code(address: dict, address_path: str) -> dict:
    ...
    try:
        ...
        if country not in NO_POSTAL_CODE_COUNTRY_CODES and not postal_code:
            return {"error": _("Postal code is required."), "path": f"{address_path}/postalCode"}
    except LookupError:
...
def validate_effective_date(filing_json: dict) -> list:
    ...
    try:
        ...
        msg.append(
            {
                "error": f"{filing_effective_date} is an invalid ISO format for effectiveDate.",
    ...
    for party in parties:
        ...
        if matched_db_party:
            ...
            if not is_same_str(email, matched_db_party.email):
                changes["email"] = {"old": normalize_str(matched_db_party.email), "new": normalize_str(email)}
            ...
            changes["name"] = {"old": old_name, "new": new_name, "changed": name_changed}
            # Mailing address comparison
            db_mailing_address = (
                Address.find_by_id(matched_db_party.mailing_address_id) if matched_db_party.mailing_address_id else None
            )
            old_mailing = (
                {
            ...
            db_delivery_address = (
                Address.find_by_id(matched_db_party.delivery_address_id)
                if matched_db_party.delivery_address_id
                else None
            )
...
def validate_staff_payment(filing_json: dict) -> bool:
    ...
    if (
        "routingSlipNumber" in header
        or "bcolAccountNumber" in header
        or "datNumber" in header
        or "waiveFees" in header
        or "priority" in header
    ):
        return True
    return False
...
def validate_certified_by(filing_json: dict) -> list:
    ...
    if certified_by.strip() and certified_by != certified_by.strip():
        msg.append(
            {"error": "Certified by field cannot start or end with whitespace.", "path": "/filing/header/certifiedBy"}
        )
