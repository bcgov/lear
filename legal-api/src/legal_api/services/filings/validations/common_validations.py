...
def validate_series(item, memoize_names, filing_type, index) -> Error:
    ...
    for series_index, series in enumerate(item.get("series", [])):
        ...
        if series["name"] in memoize_names:
            msg.append(
                {
                    "error": "Share series %s name already used in a share class or series." % series["name"],
                    "path": err_path,
                }
            )
        else:
...
def validate_shares(item, memoize_names, filing_type, index) -> Error:
    ...
    if item["name"] in memoize_names:
        ...
        msg.append(
            {"error": "Share class %s name already used in a share class or series." % item["name"], "path": err_path}
        )
    else:
...
def validate_pdf(file_key: str, file_key_path: str) -> Optional[list]:
    ...
    try:
        ...
        if any(x.mediabox.width != 612 or x.mediabox.height != 792 for x in pdf_reader.pages):
            msg.append(
                {"error": _("Document must be set to fit onto 8.5” x 11” letter-size paper."), "path": file_key_path}
            )
