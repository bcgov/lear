...
class NrTypeMap(Resource):
    ...
    def get():
        ...
        # get query params
        nr_type_cd = (request.args.get("nrTypeCd", None).upper()) if request.args.get("nrTypeCd", None) else None
        entity_type_cd = (
            (request.args.get("entityTypeCd", None).upper()) if request.args.get("entityTypeCd", None) else None
        )
        request_action_cd = (
            (request.args.get("requestActionCd", None).upper()) if request.args.get("requestActionCd", None) else None
        )
        ...
        for item in request_type_mapping:
            ...
            if match0 and match1 and match2:
                rv.append({"nrTypeCd": item[0], "entityTypeCd": item[1], "requestActionCd": item[2]})
