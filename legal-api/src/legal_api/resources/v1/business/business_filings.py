...
class ListFilingResource(Resource):
    ...
    def get(identifier, filing_id=None):  # pylint: disable=too-many-return-statements,too-many-branches;
        ...
        if not authorized(identifier, jwt, action=["view"]):
            return jsonify(
                {"message": f"You are not authorized to view filings for {identifier}."}
            ...
            if filing_json["filing"]["header"]["status"] == Filing.Status.PENDING.value:
                try:
                    ...
                    pay_response = requests.get(
                        url=f"{payment_svc_url}/{filing_json['filing']['header']['paymentToken']}", headers=headers
                    )
                    pay_details = {
        ...
        filings = CoreFiling.get_filings_by_status(
            business.id, [Filing.Status.COMPLETED.value, Filing.Status.PAID.value]
        )
    ...
    def put(identifier, filing_id):  # pylint: disable=too-many-return-statements,too-many-locals
        ...
        # check authorization
        response, response_code = ListFilingResource._check_authorization(identifier, json_input, business, filing_id)
        if response:
            return response, response_code

        # get query params
        draft = (request.args.get("draft", None).lower() == "true") if request.args.get("draft", None) else False
        only_validate = (
            (request.args.get("only_validate", None).lower() == "true")
            if request.args.get("only_validate", None)
            else False
        )
    ...
    def _create_deletion_locked_response(identifier, filing):
        ...
        if (
            ...
            and business.legal_type in [lt.value for lt in (Business.LIMITED_COMPANIES + Business.UNLIMITED_COMPANIES)]
        ):
            response = (
    ...
    def patch(identifier, filing_id=None):
        ...
        try:
            ...
            return {
                "errors": [{"message": "Unable to cancel payment for the filing."}]
    ...
    def complete_filing(business, filing, draft, payment_account_id) -> Tuple[dict, int]:
        ...
        if not draft:
            ...
            pay_msg, pay_code = ListFilingResource._create_invoice(
    ...
    def _check_authorization(
        identifier, filing_json: dict, business: Business, filing_id: int = None
    ) -> Tuple[dict, int]:
        ...
        legal_type = (
            business.legal_type if business else filing_json["filing"][filing_type]["nameRequest"].get("legalType")
        )
        ...
        if not authorized(identifier, jwt, action=["edit"]) or not is_allowed(
            ...
            return jsonify(
                {"message": f"You are not authorized to submit a filing for {identifier}."}
            ), HTTPStatus.UNAUTHORIZED
    ...
    def _save_filing(
        ...
        if business_identifier.startswith("T"):
            ...
            if client_request.method == "PUT":
                rv = (
                    db.session.query(Business, Filing)
    ...
    def _get_filing_types(business: Business, filing_json: dict):
        ...
        if any("correction" in x for x in filing_json["filing"].keys()):
            ...
            any(filing_type in x for x in ["dissolution"])
            and filing_json["filing"]["dissolution"]["dissolutionType"] == "voluntary"
        ):
                ...
                if k == "changeOfDirectors":
                    ...
                    for director in filing_json["filing"][k].get("directors"):
                        ...
                        if not all(change in free_changes for change in director.get("actions", [])):
                            ...
                            break
                    filing_type_code = (
                        Filing.FILINGS[k].get("free", {}).get("codes", {}).get(legal_type)
                        if free
                        else Filing.FILINGS[k].get("codes", {}).get(legal_type)
                    )
    ...
    def _create_invoice(
        ...
        payload = {
            ...
            "filingInfo": {"filingTypes": filing_types},
        }
        ...
        if status is None:
            ...
            for filing in pending_filings:
                ...
                if (
                    ...
                    and (filing.filing_type != "correction" or business.legal_type != business.LegalTypes.COOP.value)
                ):
                    ...
                    if filing.filing_type == "correction":
                        colin_ids = ColinEventId.get_by_filing_id(
                            filing_json["filing"]["correction"]["correctedFilingId"]
                        )
                        if not colin_ids:
                            continue
