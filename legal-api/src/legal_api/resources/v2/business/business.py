...
def get_businesses(identifier: str):
    ...
    if flags.is_on("enable-auth-v2-business") and not authorized(identifier, jwt, action=["view"]):
        ...
        return jsonify({"message": f"You are not authorized to view business {identifier}."}), HTTPStatus.UNAUTHORIZED
    ...
    if q_account and jwt.has_one_of_roles([SYSTEM_ROLE, ACCOUNT_IDENTITY]):
        ...
        current_app.logger.info(
            "VALID account request, for accountId: %s, by: %s, jwt: %s, for org account: %s",
            q_account,
            g.jwt_oidc_token_info.get("preferred_username"),
...
def search_businesses():
    ...
    try:
        ...
        bus_results, bus_hasmore = BusinessSearchService.get_search_filtered_businesses_results(
            business_json=json_input, identifiers=business_identifiers, search_filters=search_filters
        ) or ([], False)
        draft_results, draft_hasmore = BusinessSearchService.get_search_filtered_filings_results(
            business_json=json_input, identifiers=temp_identifiers, search_filters=search_filters
        ) or ([], False)
        has_more = bus_hasmore or draft_hasmore
        return jsonify(
            {"businessEntities": bus_results, "draftEntities": draft_results, "hasMore": has_more}
        ), HTTPStatus.OK
...
def get_filing_details():
    ...
    return jsonify({"count": len(results), "entityDetails": results})
