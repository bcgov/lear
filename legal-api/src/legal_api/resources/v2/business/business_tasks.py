        ...
        if not Filing.get_incomplete_filings_by_type(business.id, "transition"):
            ...
            if not restorations:
                ...
                current_app.logger.error(
                    f"Error - Business id: {business.id}. TRANSITION_NOT_FILED warning and no restoration filing on record."
                )
                return tasks
            ...
            transition_order, transition_enabled = _find_task_order_for_ta(
                tasks, order, last_restoration_date, last_ar_date
            )
            ...
            tasks.append(
                create_transition_todo(business, transition_order, (transition_enabled and not pending_filings))
            )
            order += 1
...
def add_tr_tasks(business: Business, tasks: list, order: int, pending_tr_type: str = None):
    ...
    if tr_required and (tr_start_date := current_app.config.get("TR_START_DATE", None)):
        ...
        if not pending_tr_type:
            ...
            if last_restoration_datetime and not (
                initial_filing and initial_filing.effective_date > last_restoration_datetime
            ):
                pending_tr_type = "initial"
                tasks, order = _add_tr_task(
                    tasks, order, True, business, "initial", last_restoration_datetime + datedelta.datedelta(months=6)
                )

            elif business.founding_date > tr_start_datetime and not initial_filing:
                pending_tr_type = "initial"
                tasks, order = _add_tr_task(
                    tasks, order, True, business, "initial", business.founding_date + datedelta.datedelta(months=6)
                )
...
def _add_tr_task(
    tasks: list,
    order: int,
    enabled: bool,  # pylint: disable=too-many-arguments
    business: Business,
    sub_type: str,
    due_date: datetime,
    year: int = None,
...
def create_conversion_filing_todo(business, order, enabled):
    """Return a to-do JSON object."""
    todo = {
        "task": {"todo": {"business": business.json(), "header": {"name": "conversion", "status": "NEW"}}},
        "order": order,
        "enabled": enabled,
    }
    return todo
...
def create_transition_todo(business, order, enabled):
    ...
    todo = {
        "task": {"todo": {"business": business.json(), "header": {"name": "transition", "status": "NEW"}}},
        "order": order,
        "enabled": enabled,
    }
    return todo
