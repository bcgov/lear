...
from legal_api.models import DCConnection, DCDefinition, DCIssuedCredential, DCRevocationReason, LegalEntity, User
...
def get_connections(identifier):
    ...
    connections = DCConnection.find_by(legal_entity_id=legal_entity.id)
...
def delete_connection(identifier, connection_id):
    """Delete a connection."""
    if not LegalEntity.find_by_identifier(identifier):
        return jsonify({"message": f"{identifier} not found."}), HTTPStatus.NOT_FOUND
...
def delete_active_connection(identifier):
    """Delete an active connection for this LegalEntity."""
    if not (legal_entity := LegalEntity.find_by_identifier(identifier)):
        return jsonify({"message": f"{identifier} not found."}), HTTPStatus.NOT_FOUND

    if not (connection := DCConnection.find_active_by(legal_entity_id=legal_entity.id)):
