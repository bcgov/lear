...
def get_filings(identifier: str, filing_id: Optional[int] = None):
    ...
    if filing_id or identifier.startswith("T"):
        ...
        if flags.is_on("enable-auth-v2-filing") and not authorized(identifier, jwt, action=["view"]):
            ...
            return jsonify({"message": f"You are not authorized to view filing {identifier}."}), HTTPStatus.UNAUTHORIZED
...
@pydantic_validate()
def saving_filings(
    body: FilingModel,  # pylint: disable=too-many-return-statements,too-many-locals,too-many-branches
    query: QueryModel,
    identifier,
    ...
    if Filing.FILINGS[filing.filing_type].get("staffApprovalRequired", False) and filing.status in [
...
def delete_filings(identifier, filing_id=None):
    ...
    if not authorized(identifier, jwt, action=["edit"]):
        return jsonify(
            {"message": _("You are not authorized to delete a filing for:") + identifier}
...
class ListFilingResource:  # pylint: disable=too-many-public-methods
    ...
    def get_single_filing(identifier: str, filing_id: int):
        ...
        if (
            (submitter := rv.storage.filing_submitter)
            and submitter.username
            and not rv.redact_submitter(rv.storage.submitter_roles, jwt)
        ):
        ...
        ] and (review_result := ReviewResult.get_last_review_result(rv.id)):
    ...
    def get_ledger_listing(identifier: str, user_jwt: JwtManager):
        ...
        if str(request.accept_mimetypes) == "application/pdf":
            return jsonify(
                {"message": _("Cannot return a single PDF of multiple filing submissions.")}
        ...
        filings = CoreFiling.ledger(
            business.id,
            jwt=user_jwt,
            statuses=[Filing.Status.COMPLETED.value, Filing.Status.PAID.value, Filing.Status.WITHDRAWN.value],
            start=ledger_start,
            size=ledger_size,
    ...
    def is_filing_locked_from_deletion(business, filing: Filing):
        ...
        if filing.status != Filing.Status.DRAFT.value:
            ...
            if filing.filing_type == "alteration" and business.legal_type in [
                lt.value for lt in (Business.LIMITED_COMPANIES + Business.UNLIMITED_COMPANIES)
            ]:
    ...
    def get_notice_of_withdrawal(filing_id: str = None):
        ...
        filing = db.session.query(Filing).filter(Filing.withdrawn_filing_id == filing_id).one_or_none()
    ...
    def put_basic_checks(identifier, filing, client_request, business) -> Tuple[dict, int]:
        ...
        if (
            filing_type not in CoreFiling.NEW_BUSINESS_FILING_TYPES + [CoreFiling.FilingTypes.NOTICEOFWITHDRAWAL]
    ...
    def check_authorization(
        identifier, filing_json: dict, business: Business, filing: Filing = None
    ) -> Tuple[dict, int]:
        ...
        if flags.is_on("enable-permissions-for-action") and not PermissionService.has_permissions_for_action(
            filing_type, legal_type, filing_sub_type
        ):
            return (
                jsonify(
                    {
        ...
        try:
            ...
            if (
                epoch_filing := Filing.get_filings_by_status(
                    business_id=business.id, status=[Filing.Status.EPOCH.value]
                )
                ...
                payload = {"filingMessage": {"filingIdentifier": filing.id}}
                publish_to_queue(
                    data=payload,
    ...
    def _hide_in_ledger(filing: Filing) -> bool:
        ...
        if (
            filing.filing_type == "adminFreeze"
            or (filing.filing_type == "dissolution" and filing.filing_sub_type == "involuntary")
            or (jwt.validate_roles([SYSTEM_ROLE]) and hide_in_ledger == "true")
        ):
            return True
    ...
    def _save_colin_event_ids(filing: Filing, business: Union[Business, RegistrationBootstrap]):
        try:
            filing.filing_date = datetime.datetime.fromisoformat(
                filing.filing_json["filing"]["header"]["colinDate"] or filing.filing_json["filing"]["header"]["date"]
            )
            ...
            current_app.logger.error("Business:%s missing filing/header values, unable to save", business.identifier)
    ...
    def get_filing_types(business: Business, filing_json: dict):  # pylint: disable=too-many-branches
        ...
        elif business.identifier.startswith("T") and filing_type == CoreFiling.FilingTypes.NOTICEOFWITHDRAWAL:
            legal_type = filing_json["filing"].get("business", None).get("legalType")
        else:
            legal_type = business.legal_type
        ...
        if any("correction" in x for x in filing_json["filing"].keys()):
            ...
            filing_types.extend(
                ListFilingResource.get_filing_types_for_dissolution(
                    filing_json, legal_type, priority_flag, waive_fees_flag
                )
            )
        elif filing_type in ("adminFreeze", "putBackOff", "putBackOn", "registrarsNotation", "registrarsOrder"):
            ...
            for k in filing_json["filing"].keys():
                ...
                if k == "changeOfDirectors":
                    filing_type_code = ListFilingResource.get_filing_types_for_cod(
                        filing_json, legal_type, filing_type_code
                    )
                elif k == "alteration":
                    filing_type_code = ListFilingResource.get_filing_code_for_alteration(
                        business, filing_json, filing_type_code
                    )
    ...
    @staticmethod
    def create_invoice(
        business: Business,  # pylint: disable=too-many-locals,too-many-branches,too-many-statements
        filing: Filing,
        filing_types: list,
        user_jwt: JwtManager,
        ...
        if filing.filing_type in CoreFiling.NEW_BUSINESS_FILING_TYPES:
            ...
            corp_type = (
                business.legal_type if business.legal_type else filing.json["filing"]["business"].get("legalType")
            )
        # deal with withdrawing a new business filing
            ...
            mailing_address, corp_type, legal_name = ListFilingResource._get_address_from_withdrawn_new_business_filing(
                business, filing
            )
    ...
    def delete_from_minio(filing_type: str, filing_json: dict):
        ...
        if (
            filing_type == Filing.FILINGS["incorporationApplication"].get("name")
            and (
                cooperative := filing_json.get("filing", {})
                .get("incorporationApplication", {})
                .get("cooperative", None)
            )
        ) or (
            filing_type == Filing.FILINGS["alteration"].get("name")
            and (cooperative := filing_json.get("filing", {}).get("alteration", {}))
        ):
            ...
            affidavit_file_key := filing_json.get("filing", {}).get("dissolution", {}).get("affidavitFileKey", None)
        ):
            MinioService.delete_file(affidavit_file_key)
        elif filing_type == Filing.FILINGS["courtOrder"].get("name") and (
            file_key := filing_json.get("filing", {}).get("courtOrder", {}).get("fileKey", None)
        ):
    ...
    def modify_filing_json(filing_json, filing: Filing = None, for_validation=False):
        ...
        if filing_type == Filing.FILINGS["continuationIn"].get("name"):
            if for_validation and (
                not filing or filing.status in [Filing.Status.DRAFT.value, Filing.Status.CHANGE_REQUESTED.value]
            ):
            ...
            if filing and filing.status == Filing.Status.APPROVED.value:
                ...
                filing_json["filing"][filing_type]["authorization"] = filing.filing_json["filing"][filing_type][
                    "authorization"
                ]
                filing_json["filing"][filing_type]["nameRequest"] = filing.filing_json["filing"][filing_type][
                    "nameRequest"
                ]
                filing_json["filing"][filing_type]["foreignJurisdiction"] = filing.filing_json["filing"][filing_type][
                    "foreignJurisdiction"
                ]
