...
def get_business_documents(identifier: str, document_name: str = None):
    ...
    if (
        not flags.is_on("enable-business-summary-for-migrated-corps", user=rc.user, account_id=rc.account_id)
        and business.is_tombstone
        and business.legal_type in Business.CORPS
        and document_name == "summary"
    ):
...
def _get_coop_documents_and_info(business):
    ...
    def filter_resolutions(filings, filing_type, key):
        return [f for f in filings if f.filing_json["filing"].get(filing_type, {}).get(key) is True]

    rules_resolutions = filter_resolutions(sr_filings, "alteration", "rulesInResolution") + filter_resolutions(
        cr_filings, "correction", "rulesInResolution"
    )

    memo_resolutions = filter_resolutions(sr_filings, "alteration", "memorandumInResolution") + filter_resolutions(
        cr_filings, "correction", "memorandumInResolution"
    )
    ...
    def set_doc_info(doc_type, doc_label):
        if doc := Document.find_by_business_id_and_type(business.id, doc_type.value):
            ...
            doc_url = url_for(
                "API2.get_documents", identifier=business.identifier, filing_id=filing.id, legal_filing_name=None
            )
            ...
            info[doc_label] = {
                "key": doc.file_key,
                "name": (
                    f"{business.identifier} - {doc_label.replace('certified', 'Certified ')} - {filing_date_str}.pdf"
                ),
