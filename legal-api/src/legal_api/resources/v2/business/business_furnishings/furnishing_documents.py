...
def get_furnishing_document(identifier: str, furnishing_id: int):
    ...
    if (
        not (furnishing := Furnishing.find_by_id(furnishing_id))
        or furnishing.business_id != business.id
        or furnishing.furnishing_type == Furnishing.FurnishingType.GAZETTE
    ):
        return jsonify(
            message=get_error_message(
                ErrorCode.FURNISHING_NOT_FOUND, **{"furnishing_id": furnishing_id, "identifier": identifier}
            )
        ), HTTPStatus.NOT_FOUND
    ...
    if "application/pdf" in request.accept_mimetypes:
        try:
            ...
            return current_app.response_class(response=pdf, status=HTTPStatus.OK, mimetype="application/pdf")
        except Exception:
            return jsonify({"message": "Unable to get furnishing document."}), HTTPStatus.INTERNAL_SERVER_ERROR
