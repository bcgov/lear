...
def diff_list_with_id(json1, json2, path: List[str] = None, ignore_keys: List[str] = None) -> Optional[List[Node]]:
    ...
    """
    if not (isinstance(json1, MutableSequence) and isinstance(json2, MutableSequence)):
        return None
    ...
    memoize = []
    for row1 in json1:
        if not (row1_id := row1['id']):  # pylint: disable=superfluous-parens; linter confused by the walrus :=
            continue
