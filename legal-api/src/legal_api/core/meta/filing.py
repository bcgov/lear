...
FILINGS: Final = {
    ...
    "agmExtension": {
        ...
        "additional": [
            {"types": ["BC", "BEN", "CC", "ULC", "C", "CBEN", "CCC", "CUL"], "outputs": ["letterOfAgmExtension"]},
        ],
    },
    "agmLocationChange": {
        ...
        "additional": [
            {"types": ["BC", "BEN", "CC", "ULC", "C", "CBEN", "CCC", "CUL"], "outputs": ["letterOfAgmLocationChange"]},
        ],
    },
    "alteration": {
        "name": "alteration",
        ...
        "additional": [
            {"types": ["BC", "BEN", "CC", "ULC", "C", "CBEN", "CCC", "CUL"], "outputs": ["noticeOfArticles"]},
        ],
    },
    "amalgamationApplication": {
        "name": "amalgamationApplication",
        "additional": [
            {"types": ["BC", "BEN", "CC", "ULC"], "outputs": ["noticeOfArticles", "certificateOfAmalgamation"]},
        ],
        "regular": {
    ...
    "changeOfRegistration": {
        ...
        "displayName": {"SP": "Change of Registration Application", "GP": "Change of Registration Application"},
    ...
    "consentContinuationOut": {
        ...
        "additional": [
            {"types": ["BC", "BEN", "CC", "ULC", "C", "CBEN", "CCC", "CUL"], "outputs": ["letterOfConsent"]},
        ],
    },
    "continuationIn": {
        "name": "continuationIn",
        ...
        "additional": [
            {"types": ["C", "CBEN", "CCC", "CUL"], "outputs": ["noticeOfArticles", "certificateOfContinuation"]},
        ],
    },
    "continuationOut": {
    ...
    "dissolution": {
        ...
        "additional": [
            ...
            {"types": ["BC", "BEN", "CC", "ULC", "C", "CBEN", "CCC", "CUL"], "outputs": ["certificateOfDissolution"]},
        ],
        "voluntary": {
    ...
    "incorporationApplication": {
        ...
        "additional": [
            {"types": ["CP"], "outputs": ["certificateOfIncorporation", "certifiedRules", "certifiedMemorandum"]},
            {"types": ["BC", "BEN", "CC", "ULC"], "outputs": ["noticeOfArticles", "certificateOfIncorporation"]},
        ],
    },
    "intentToLiquidate": {
    ...
    "registration": {
        "name": "registration",
        "title": "Registration",
        "displayName": {"SP": "BC Sole Proprietorship Registration", "GP": "BC General Partnership Registration"},
...
class FilingMeta:  # pylint: disable=too-few-public-methods
    ...
    def display_name(business: Business, filing: FilingStorage) -> Optional[str]:
        ...
        if filing.meta_data and (display_name := filing.meta_data.get("colinDisplayName")):
            return display_name
        # if there is no lookup
        if not (names := FILINGS.get(filing.filing_type, {}).get("displayName")):
            if not (
                filing.filing_sub_type
                and (names := FILINGS.get(filing.filing_type, {}).get(filing.filing_sub_type, {}).get("displayName"))
            ):
        ...
        if filing.transaction_id and (bus_rev_temp := VersionService.get_business_revision_obj(filing, business.id)):
        ...
        if filing.filing_type == "correction":
            ...
            if correction.get("commentOnly") and business.legal_type in Business.CORPS:
                ...
                if corrected_filing_type == "amalgamationApplication":
                    ...
                    and business.legal_type != Business.LegalTypes.COOP.value
                ):
    ...
    def get_static_documents(filing, url_prefix):
        ...
        if filing.filing_type == "continuationIn":
            ...
            if file_key := continuation_in.get("affidavitFileKey"):
                outputs.append(
                    {"name": "Unlimited Liability Corporation Information", "url": f"{url_prefix}/{file_key}"}
                )
            if authorization_files := continuation_in.get("authorizationFiles"):
                for file in authorization_files:
                    file_key = file.get("fileKey")
                    outputs.append({"name": file.get("fileName"), "url": f"{url_prefix}/{file_key}"})
        return outputs
