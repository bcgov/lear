...
FILINGS: Final = {
    ...
    "courtOrder": {"name": "courtOrder", "title": "Court Order", "displayName": "Court Order", "code": "NOFEE"},
    "dissolution": {
        "name": "dissolution",
        "additional": [
...
class FilingMeta:  # pylint: disable=too-few-public-methods
    ...
    def alter_outputs_alteration(filing, outputs):
        ...
        if filing.filing_type == "alteration":
            if filing.meta_data.get("alteration", {}).get("toBusinessName"):
    ...
    def alter_outputs_correction(filing, business, outputs):
        ...
        if filing.filing_type == "correction":
            if filing.meta_data.get("correction", {}).get("toBusinessName"):
                outputs.add("certificateOfNameChange")
    ...
    def alter_outputs_dissolution(filing, outputs):
        ...
        if filing.filing_type == "dissolution":
            ...
            if filing.filing_sub_type == "voluntary" and filing.json_legal_type == LegalEntity.EntityTypes.COOP:
