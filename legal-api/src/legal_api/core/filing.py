...
class Filing:
    ...
    def get_json(self) -> Optional[Dict]:
        ...
        elif self._storage.status in [Filing.Status.APPROVED.value, Filing.Status.PAID.value, Filing.Status.PENDING.value]:
    ...
    def get_document_list(business,  # pylint: disable=too-many-locals disable=too-many-branches
        ...
        if filing.status == [Filing.Status.PAID, Filing.Status.APPROVED] and \
