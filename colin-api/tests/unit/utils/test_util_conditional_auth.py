...
def mock_end_point_func(*args, **kwargs):  # pylint: disable=W0613;
    """Mock endpoint function."""
    return "Endpoint Function"


# Mock auth decorator
def mock_auth_decorator(roles):  # pylint: disable=W0613;
    """Mock auth decorator function."""
    def decorator(f):
        @wraps(f)
...
def test_auth_enabled_executes_auth_decorator():
    """
    Assert that a mock endpoint using conditional_auth invokes the auth decorator(mock_auth_decorator).

    In this scenario, colin api auth is turned on(disable-colin-api-auth = False).
    """
...
def test_auth_disabled_skips_auth_decorator():
    """
    Assert that a mock endpoint using conditional_auth does not invoke the auth decorator(mock_auth_decorator).

    In this scenario, colin api auth is turned off(disable-colin-api-auth = True).
    """
