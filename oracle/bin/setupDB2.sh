...
/bin/bash $SETUP_DIR/untarDB2.sh 2>&1 >> $UNTAR_LOG
...
/bin/bash $SETUP_DIR/configDB2.sh 2>&1 >> $CONFIG_LOG
...
then
    ...
    echo "configDB2.sh is done at $SECONDS sec"
    echo "configDB2.sh is done at $SECONDS sec" >> $SETUP_LOG
    echo ""
    echo "" >> $SETUP_LOG
