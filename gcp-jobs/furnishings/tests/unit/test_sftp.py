...
def test_get(sftpserver, sftpconnection):
    ...
    with sftpserver.serve_content({server_dir: {file_name: file_content}}), sftpconnection as sftpclient, sftpclient.open(path.join(server_dir, file_name)) as file:
...
def test_put(tmp_path, sftpserver, sftpconnection):
    ...
    with sftpserver.serve_content({server_dir: {}}), sftpconnection as sftpclient:
