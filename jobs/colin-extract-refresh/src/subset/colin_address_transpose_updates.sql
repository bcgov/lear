...
DROP PROCEDURE IF EXISTS TARGET_SCHEMA.colin_address_transpose_city();
DROP PROCEDURE IF EXISTS TARGET_SCHEMA.colin_address_duplicate_region();
DROP PROCEDURE IF EXISTS TARGET_SCHEMA.colin_address_transpose_region();
DROP PROCEDURE IF EXISTS TARGET_SCHEMA.colin_address_duplicate_pcode();
DROP PROCEDURE IF EXISTS TARGET_SCHEMA.colin_address_transpose_pcode();
DROP PROCEDURE IF EXISTS TARGET_SCHEMA.colin_address_duplicate_country();
DROP PROCEDURE IF EXISTS TARGET_SCHEMA.colin_address_transpose_country();


--
-- Try and extract a Canada or US postal code from address text: city, then address_line_3, then address_line_2
--
create or replace function TARGET_SCHEMA.colin_update_address_pcode(v_text character varying,
                                                             v_remove boolean,
...
--
create or replace function TARGET_SCHEMA.colin_update_address_country(v_text character varying, v_remove boolean) returns character varying
  immutable
  language plpgsql
as $$
declare
  countries text[] := ARRAY[ARRAY['U.S.A.', 'US'],
                            ...
                            ARRAY['AUSTRALIA','AU'],
                            ARRAY['BARBADOS','BB'],
                            ARRAY['PEOPLES RETARGET_SCHEMA OF CHINA','CN'],
                            ARRAY['P R OF CHINA','CN'],
                            ARRAY['TAIWAN RETARGET_SCHEMA OF CHINA','TW'],
                            ARRAY['CHINA','CN'],
                            ARRAY['PRC','CN'],
                            ARRAY['ENGLAND','GB'],
                            ARRAY['UK','GB'],
  ...
  if v_text is null or LENGTH(v_text) < 2 then
    ...
    foreach country slice 1 in array countries
     loop
       ...
       elsif country[1] in ('PEOPLES RETARGET_SCHEMA OF CHINA', 'P R OF CHINA', 'TAIWAN RETARGET_SCHEMA OF CHINA', 'EAST MALAYSIA', 'HONG KONG') and
...
--
create or replace function TARGET_SCHEMA.colin_update_address_ca_province(v_text character varying, v_remove boolean) returns character varying
  immutable
  language plpgsql
as $$
declare
...
--
create or replace function TARGET_SCHEMA.colin_update_address_us_state(v_text character varying, v_remove boolean) returns character varying
  immutable
  language plpgsql
as $$
declare
...
--
CREATE OR REPLACE PROCEDURE TARGET_SCHEMA.colin_address_transpose_country(
...
--
CREATE OR REPLACE PROCEDURE TARGET_SCHEMA.colin_address_duplicate_country(
    OUT row_occurrences bigint
)
LANGUAGE plpgsql
AS $$
...
--
CREATE OR REPLACE PROCEDURE TARGET_SCHEMA.colin_address_transpose_pcode(
    OUT row_occurrences bigint
)
LANGUAGE plpgsql
AS $$
...
--
CREATE OR REPLACE PROCEDURE TARGET_SCHEMA.colin_address_duplicate_pcode(
    OUT row_occurrences bigint
)
LANGUAGE plpgsql
AS $$
...
--
CREATE OR REPLACE PROCEDURE TARGET_SCHEMA.colin_address_transpose_region(
    OUT row_occurrences bigint
)
LANGUAGE plpgsql
AS $$
...
--
CREATE OR REPLACE PROCEDURE TARGET_SCHEMA.colin_address_duplicate_region(
    OUT row_occurrences bigint
)
LANGUAGE plpgsql
AS $$
...
--
CREATE OR REPLACE PROCEDURE TARGET_SCHEMA.colin_address_transpose_city(
    OUT row_occurrences bigint
)
LANGUAGE plpgsql
AS $$
...
--
CREATE OR REPLACE PROCEDURE TARGET_SCHEMA.colin_address_transpose_cleanup(
    OUT row_occurrences bigint
)
LANGUAGE plpgsql
AS $$
...
-- call colin_address_transpose(NULL);
--
CREATE OR REPLACE PROCEDURE TARGET_SCHEMA.colin_address_transpose(
    OUT result json
)
LANGUAGE plpgsql
AS $$
...
BEGIN
    CALL TARGET_SCHEMA.colin_address_transpose_country(v_country_rows);
    CALL TARGET_SCHEMA.colin_address_duplicate_country(v_duplicate_country_rows);
    CALL TARGET_SCHEMA.colin_address_transpose_pcode(v_pcode_rows);
    CALL TARGET_SCHEMA.colin_address_duplicate_pcode(v_duplicate_pcode_rows);
    CALL TARGET_SCHEMA.colin_address_transpose_region(v_region_rows);
    CALL TARGET_SCHEMA.colin_address_duplicate_region(v_duplicate_region_rows);
    CALL TARGET_SCHEMA.colin_address_transpose_city(v_city_rows);
    CALL TARGET_SCHEMA.colin_address_transpose_cleanup(v_cleanup_rows);
...
COMMENT ON PROCEDURE TARGET_SCHEMA.colin_address_transpose() IS
