-- transfer TARGET_SCHEMA.a chunk (or a whole subset) of corps from the SOURCE Oracle DB (cprd) into the TARGET Postgres extract DB (cprd_pg).
--
...
--   Instead, Oracle SELECTs emit boolean-friendly 'true'/'false' strings for Postgres boolean columns.
...
--   Instead, load the requested corp set and purge BCOMPS-excluded corps ONCE in Postgres after the transfer TARGET_SCHEMA.suite completes
...
-- corp_name
transfer TARGET_SCHEMA.corp_name from cprd using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where &oracle_corp_num_predicate
...
-- corp_state
transfer TARGET_SCHEMA.corp_state from cprd using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where &oracle_corp_num_predicate
...
-- filing_user
transfer TARGET_SCHEMA.filing_user from cprd using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where &oracle_corp_num_predicate
...
-- office
transfer TARGET_SCHEMA.office from cprd using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where &oracle_corp_num_predicate
...
-- corp_comments
transfer TARGET_SCHEMA.corp_comments from cprd using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where &oracle_corp_num_predicate
...
-- ledger_text
transfer TARGET_SCHEMA.ledger_text from cprd using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where &oracle_corp_num_predicate
...
-- corp_party
transfer TARGET_SCHEMA.corp_party from cprd using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where &oracle_corp_num_predicate
...
transfer TARGET_SCHEMA.corp_party_relationship from cprd using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where &oracle_corp_num_predicate
...
-- offices_held
transfer TARGET_SCHEMA.offices_held from cprd using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where &oracle_corp_num_predicate
...
-- completing_party
transfer TARGET_SCHEMA.completing_party from cprd using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where &oracle_corp_num_predicate
...
-- submitting_party
transfer TARGET_SCHEMA.submitting_party from cprd using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where &oracle_corp_num_predicate
...
-- corp_flag
transfer TARGET_SCHEMA.corp_flag from cprd using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where &oracle_corp_num_predicate
...
-- cont_out
transfer TARGET_SCHEMA.cont_out from cprd using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where &oracle_corp_num_predicate
...
transfer TARGET_SCHEMA.jurisdiction from cprd using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where &oracle_corp_num_predicate
...
-- resolution
transfer TARGET_SCHEMA.resolution from cprd using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where &oracle_corp_num_predicate
...
-- share_struct
transfer TARGET_SCHEMA.share_struct from cprd using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where &oracle_corp_num_predicate
...
-- notification_resend
transfer TARGET_SCHEMA.notification_resend from cprd using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where &oracle_corp_num_predicate
...
-- party_notification
transfer TARGET_SCHEMA.party_notification from cprd using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where &oracle_corp_num_predicate
...
-- payment
transfer TARGET_SCHEMA.payment from cprd using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where &oracle_corp_num_predicate
