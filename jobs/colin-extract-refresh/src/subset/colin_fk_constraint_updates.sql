...
CREATE TABLE IF NOT EXISTS TARGET_SCHEMA.colin_dropped_fks
(
...
CREATE OR REPLACE PROCEDURE TARGET_SCHEMA.colin_fk_drop_all(
...
BEGIN
    ...
    FOR fk IN
        ...
        LEFT JOIN TARGET_SCHEMA.colin_dropped_fks AS dropped
    ...
    LOOP
        ...
        INSERT INTO TARGET_SCHEMA.colin_dropped_fks (
            table_schema,
            table_name,
            conname,
            condef
...
CREATE OR REPLACE PROCEDURE TARGET_SCHEMA.colin_fk_restore_all(
...
BEGIN
    ...
    FOR fk IN
        SELECT
            ...
            conname,
            condef
        FROM TARGET_SCHEMA.colin_dropped_fks
        ORDER BY table_schema, table_name, conname
    LOOP
        -- New captures contain qualified relation names. Keep TARGET_SCHEMA on a
        ...
        PERFORM pg_catalog.set_config('search_path', 'pg_catalog, TARGET_SCHEMA', true);
        ...
        DELETE FROM TARGET_SCHEMA.colin_dropped_fks
...
COMMENT ON TABLE TARGET_SCHEMA.colin_dropped_fks IS
'Foreign key definitions captured while a COLIN extract refresh is in progress.';

COMMENT ON PROCEDURE TARGET_SCHEMA.colin_fk_drop_all() IS
'Captures and drops every foreign key on ordinary tables in TARGET_SCHEMA, committing each drop for resumability.';

COMMENT ON PROCEDURE TARGET_SCHEMA.colin_fk_restore_all() IS
