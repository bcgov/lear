...
def tmpl_default_bundle(repo_root: Path, schema: str) -> tmpl_TemplateBundle:
    ...
    disable_triggers = tmpl_TemplateSpec(
        ...
        required_tokens=(TMPL_TOKEN_SCHEMA),
        schema=schema,
    )
    enable_triggers = tmpl_TemplateSpec(
...
def tmpl_schema_text(spec: tmpl_TemplateSpec) -> str:
    if not spec.schema:
        raise SystemExit(f"Missing required template: {spec.name}\nPath: {spec.path}")
    text = spec.schema
    return text
...
def tmpl_render_schema(template_text: str) -> str:
    out = template_text
    return out
...
def gen_build_chunk_sql(
    *,
    ...
    if schema:
        rendered_schema = tmpl_render(schema, replacements=replacements)
        parts.append(rendered_schema.rstrip())
        parts.append("")
    return "\n".join(parts)
...
def run(cfg: cfg_GenerationConfig) -> int:
    ...
    disable_triggers_text = tmpl_schema_text(templates.disable_triggers)
