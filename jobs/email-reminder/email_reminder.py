...
def get_ar_fee(app: Flask, entity_type: str, token: str) -> str:
    ...
    app.logger.debug(f"fee_url: {fee_url}")
    res = requests.get(
        url=fee_url,  # pylint: disable=missing-timeout
    ...
    try:
        ...
        while pagination.items:
            ...
            for business in pagination.items:
                ar_year = (
                    business.last_ar_reminder_year if business.last_ar_reminder_year else business.founding_date.year
                ) + 1
...
async def send_outstanding_bcomps_ar_reminder(app: Flask, qsm: GcpQueue):  # pylint: disable=redefined-outer-name
    ...
    try:
        ...
        businesses = (
            db.session.query(LegalEntity)
            .filter(LegalEntity.entity_type == LegalEntity.EntityTypes.BCOMP.value, where_clause)
            .all()
        )
