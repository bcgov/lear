...
def check_for_manual_filings(application: Flask = None, token: dict = None):
    ...
    if response.status_code not in [200, 404]:
        application.logger.error(
            f"Error getting last updated colin id from \
        ...
        if last_event_id:
            ...
            for info in colin_events["events"]:
                ...
                if response.status_code == 200:
                    # check legal table
                    response = requests.get(
                        f'{legal_url}/internal/filings/colin_id/{info["event_id"]}', timeout=AccountService.timeout
                    )
                    if response.status_code == 404:
                        id_list.append(info)
...
def get_filing(event_info: dict = None, application: Flask = None, token: dict = None):
    ...
    filing_type = next(
        (x for x in filing_types if filing_typ_cd in Filing.FILING_TYPES.get(x).get("type_code_list")), None
    )

    if not filing_type:
        # pylint: disable=consider-using-f-string
        application.logger.error(
            "Error unknown filing type: {} for event id: {}".format(event_info["filing_type"], event_info["event_id"])
        )
