...
export default {
  ...
  computed: {
    ...
    filedDate () /* Date */ {
      return this.$store.state.filedDate ? new Date(this.$store.state.filedDate) : null
    },
    agmDate () /* Date */ {
      return this.$store.state.agmDate ? new Date(this.$store.state.agmDate) : null
    },
    noAGM () {
  ...
  watch: {
    ...
    directorsChange: function (val) {
      // console.log('AnnualReport directorsChange watcher fired: ', val)
      if (val) {
