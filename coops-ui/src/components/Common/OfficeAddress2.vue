...
<script lang="ts">
    ...
    export default class RegisteredOfficeAddress extends Mixins(AddressMixin, CommonMixin) {
        ...
        // The two addresses that are the current state of the BaseAddress components for registerd office.
        private regDeliveryAddress: object = {}
        private regMailingAddress: object = {}

        // The two addresses that are the current state of the BaseAddress components for record office.
        ...
        private emitAddresses (): object {
            let regDeliveryAddressFinal = Object.assign({}, this.regDeliveryAddress)
            let regMailingAddressFinal = Object.assign({}, this.regMailingAddress)
            ...
            if (this.deliveryModified) this.addAction(regDeliveryAddressFinal, ADDRESSCHANGED)
            else this.removeAction(regDeliveryAddressFinal, ADDRESSCHANGED)

            // if the mailing address has changed from the original, set action flag
            if (this.mailingModified) this.addAction(regMailingAddressFinal, ADDRESSCHANGED)
            else this.removeAction(regMailingAddressFinal, ADDRESSCHANGED)

            // if the address has changed from the original, set action flag
            if (this.deliveryModified) this.addAction(recDeliveryAddressFinal, ADDRESSCHANGED)
            else this.removeAction(recDeliveryAddressFinal, ADDRESSCHANGED)

            // if the mailing address has changed from the original, set action flag
            if (this.mailingModified) this.addAction(recMailingAddressFinal, ADDRESSCHANGED)
            else this.removeAction(recMailingAddressFinal, ADDRESSCHANGED)

            return {
                registeredOffice:
                  {
                      deliveryAddress: regDeliveryAddressFinal,
                      mailingAddress: regDeliveryAddressFinal
                  },
                recordsOffice:
                  {
                      deliveryAddress: regDeliveryAddressFinal,
        ...
        onAddressesChanged (): void {
            this.regDeliveryAddress = isEmpty(this.addresses) ? {} : this.addresses['deliveryAddress']
            this.regMailingAddress = isEmpty(this.addresses) ? {} : this.addresses['mailingAddress']
        }
        ...
        private get modified (): boolean {
            ...
            return !(
              this.isSameAddress(this.regDeliveryAddress, this.deliveryAddressOriginal) &&
              this.isSameAddress(this.regMailingAddress, this.mailingAddressOriginal))
        }
        ...
        private get mailingModified (): boolean {
            return !this.isSameAddress(this.regMailingAddress, this.mailingAddressOriginal)
        }
        ...
        private get deliveryModified (): boolean {
            return !this.isSameAddress(this.regDeliveryAddress, this.deliveryAddressOriginal)
        }
        ...
        private updateDelivery (address: object): void {
            ...
            // the property.
            Object.assign(this.regDeliveryAddress, address)
        }
        ...
        private updateMailing (address: object): void {
            // Note that we do a copy of the fields (rather than change the object reference) to prevent an infinite loop with
            // the property.
            Object.assign(this.regMailingAddress, address)
        }
        ...
        private editAddress (): void {
            this.inheritDeliveryAddress = this.isSameAddress(this.regMailingAddress, this.regDeliveryAddress)
            this.deliveryAddressTemp = { ...this.regDeliveryAddress }
            this.mailingAddressTemp = { ...this.regMailingAddress }
        ...
        private cancelEditAddress (): void {
            this.regDeliveryAddress = { ...this.deliveryAddressTemp }
            this.regMailingAddress = { ...this.mailingAddressTemp }
        ...
        private updateAddress (): void {
            if (this.inheritDeliveryAddress) {
                this.regMailingAddress = { ...this.regDeliveryAddress }
            }
            this.showAddressForm = false
            this.emitAddresses()
            this.emitModified()
        ...
        private resetAddress (): void {
            this.regDeliveryAddress = { ...this.deliveryAddressOriginal }
            this.regMailingAddress = { ...this.mailingAddressOriginal }
            this.emitAddresses()
            this.emitModified()
        }
                ...
                axios.get(url)
                  .then(response => {
                      if (response && response.data) {
                          ...
                          // Parsing the RegisteredOffice
                          const registeredOfficeDeliveryAddress = response.data.registeredOffice.deliveryAddress
                          if (registeredOfficeDeliveryAddress) {
                              registeredOfficeDeliveryAddress.actions = []
                              console.log(registeredOfficeDeliveryAddress)
                              this.deliveryAddressOriginal = { ...this.omitProp(registeredOfficeDeliveryAddress, ['addressType']) }
                              console.log(this.deliveryAddressOriginal)
                              ...
                              if (isEmpty(this.regDeliveryAddress)) {
                                  this.regDeliveryAddress = { ...this.omitProp(registeredOfficeDeliveryAddress, ['addressType']) }
                              }
                          } else {
                              console.log('loadAddressesFromApi() error - invalid Delivery Address =', registeredOfficeDeliveryAddress)
                          }

                          const registeredOfficeMailingAddress = response.data.registeredOffice.mailingAddress
                          if (registeredOfficeMailingAddress) {
                              registeredOfficeMailingAddress.actions = []

                              this.mailingAddressOriginal = { ...this.omitProp(registeredOfficeMailingAddress, ['addressType']) }
                              ...
                              if (isEmpty(this.regMailingAddress)) {
                                  this.regMailingAddress = { ...this.omitProp(registeredOfficeMailingAddress, ['addressType']) }
                              }
                          } else {
                              console.log('loadAddressesFromApi() error - invalid Mailing Address =', registeredOfficeMailingAddress)
                          }

                          // Parsing the Record office
                          const recordOfficeDeliveryAddress = response.data.recordsOffice.deliveryAddress
                          if (recordOfficeDeliveryAddress) {
                              recordOfficeDeliveryAddress.actions = []
                              console.log(recordOfficeDeliveryAddress)
                              this.deliveryAddressOriginal = { ...this.omitProp(recordOfficeDeliveryAddress, ['addressType']) }
                              console.log(this.deliveryAddressOriginal)
                              // If parent page loaded draft before this API call, don't overwrite draft delivery address.
                              // Otherwise this API call finished before parent page loaded draft, or parent page won't
                              // load draft (ie, this is a new filing) -> initialize delivery address.
                              if (isEmpty(this.regDeliveryAddress)) {
                                  this.regDeliveryAddress = { ...this.omitProp(recordOfficeDeliveryAddress, ['addressType']) }
                              }
                          } else {
                              console.log('loadAddressesFromApi() error - invalid Delivery Address =', recordOfficeDeliveryAddress)
                          }

                          const recordOfficeMailingAddress = response.data.recordsOffice.mailingAddress
                          if (recordOfficeMailingAddress) {
                              recordOfficeMailingAddress.actions = []

                              this.mailingAddressOriginal = { ...this.omitProp(recordOfficeMailingAddress, ['addressType']) }
                              // If parent page loaded draft before this API call, don't overwrite draft mailng address
                              // Otherwise this API call finished before parent page loaded draft, or parent page won't
                              // load draft (ie, this is a new filing) -> initialize mailing address.
                              if (isEmpty(this.regMailingAddress)) {
                                  this.regMailingAddress = { ...this.omitProp(recordOfficeMailingAddress, ['addressType']) }
                              }
                          } else {
                              console.log('loadAddressesFromApi() error - invalid Mailing Address =', recordOfficeMailingAddress)
                          }
