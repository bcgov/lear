    ...
    <ul class="list address-list" v-bind:class="{ 'show-address-form' : showAddressForm }">
      ...
      <li class="container">
        <div class="meta-container">
          <label></label>
          <div class="meta-container__inner">
            <span v-if="isSameAddress(deliveryAddress, mailingAddress)">Mailing Address same as above</span>
            <div v-else>
      ...
      <li v-if="entityFilter(EntityTypes.Coop)">
        <div
...
export default class RegisteredOffice extends Mixins(AddressMixin, CommonMixin, EntityFilterMixin) {
  /**
  ...
  /**
   * Boolean indicating the state of address editing passed from the parent.
   *
   */
  @Prop({ default: null })
  readonly editClose: boolean
  ...
  /**
   * Emits the state of editing.
   *
   * @returns a boolean when the addresses are in a state of editing.
   */
  @Emit('editing')
  private emitEditing (): boolean {
    if (this.editClose) {
      return true
    } else {
      return false
  ...
  /**
   * Called when editing property changes
   */
  @Watch('editClose')
  closeEditing (): void {
      ...
      axios.get(url)
        .then(response => {
          if (response && response.data) {
            const deliveryAddress = response.data.registeredOffice.deliveryAddress
            if (deliveryAddress) {
            ...
            const mailingAddress = response.data.registeredOffice.mailingAddress
            if (mailingAddress) {
