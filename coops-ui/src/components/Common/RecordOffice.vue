<template>
  <v-card flat>
    <ul class="list address-list" v-bind:class="{ 'show-address-form' : showAddressForm }">
      ...
      <li v-if="entityFilter(EntityTypes.BCorp)">
        <div
...
export default class RecordOffice extends Mixins(AddressMixin, CommonMixin, EntityFilterMixin) {
  /**
      ...
      axios.get(url)
        .then(response => {
          if (response && response.data) {
            const deliveryAddress = response.data.recordsOffice.deliveryAddress
            if (deliveryAddress) {
            ...
            const mailingAddress = response.data.recordsOffice.mailingAddress
            if (mailingAddress) {
