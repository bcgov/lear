...
class ConsentContinuationOut(db.Model):  # pylint: disable=too-few-public-methods
    ...
    legal_entities_id = db.Column('legal_entities_id', db.Integer, db.ForeignKey('legal_entities.id'))
    ...
    @staticmethod
    def get_active_cco(legal_entities_id,
                       expiry_date,
        ...
        query = db.session.query(ConsentContinuationOut). \
            filter(ConsentContinuationOut.legal_entities_id == legal_entities_id). \
