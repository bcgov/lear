...
from business_model import Address, LegalEntity, ColinEntity
from business_model.models.entity_role import EntityRole
from tests.models import factory_address, factory_legal_entity, factory_completed_filing
...
def test_valid_related_entity_save(session):
    ...
    with freeze_time(now):
        ...
        legal_entity_1 = factory_legal_entity(
            identifier="BC1111111", entity_type=LegalEntity.EntityTypes.COMP
        )
        ...
        assert partner_1.appointment_date.replace(tzinfo=None) == now.replace(
            tzinfo=None
        )
        ...
        assert partner_2.appointment_date.replace(tzinfo=None) == now.replace(
            tzinfo=None
        )
...
def test_valid_related_colin_entity_save(session):
    ...
    with freeze_time(now):
        legal_entity_gp = factory_legal_entity(
            identifier="FM1234567", entity_type=LegalEntity.EntityTypes.PARTNERSHIP
        )
        colin_entity_1 = ColinEntity(
            identifier="BC1111111", organization_name="XYZ BC LTD"
        )
        colin_entity_1.save()
        colin_entity_2 = ColinEntity(
            identifier="BC2222222", organization_name="ABC CCC"
        )
        colin_entity_2.save()
...
def test_valid_filing_entity_role_save(session):
    ...
    with freeze_time(now):
        ...
        entity_roles = legal_entity.entity_roles.filter(
            ...
            EntityRole.filing_id == ar_filing.id,
        ).all()
        ...
        assert entity_role.appointment_date.replace(tzinfo=None) == now.replace(
            tzinfo=None
        )
        ...
        assert filing_entity_role.appointment_date.replace(tzinfo=None) == now.replace(
            tzinfo=None
        )
