...
@pytest.mark.parametrize(
    "last_ar_date, entity_type, state, limited_restoration, expected",
    TEST_GOOD_STANDING_DATA,
)
def test_good_standing(
    session, last_ar_date, entity_type, state, limited_restoration, expected
):
...
def test_business_json(session):
    ...
    d["fiscalYearEndDate"] = datetime.date(
        legal_entity.fiscal_year_end_date
    ).isoformat()
    assert legal_entity.json() == d
...
def test_business_relationships_json(session):
    ...
    mailing_address = Address(
        city="Test City", address_type=Address.MAILING, legal_entity_id=legal_entity.id
    )
    ...
    delivery_address = Address(
        city="Test City", address_type=Address.DELIVERY, legal_entity_id=legal_entity.id
    )
...
def test_legal_name_firms(
    session, test_name, firm_entity_type, partner_info, expected_legal_name
):
...
def test_business_name(
    session, entity_type, legal_name, operating_name, expected_business_name
):
            ...
            [
                {
                    "identifier": "CP1234567",
                    "entityType": "CP",
                    "legalName": "CP Test XYZ",
                },
                {
                    "identifier": "BC1234567",
                    "entityType": "BEN",
                    "legalName": "BEN Test XYZ",
                },
                {
                    "identifier": "BC1234567",
                    "entityType": "BC",
                    "legalName": "BC Test XYZ",
                },
                {
                    "identifier": "BC1234567",
                    "entityType": "ULC",
                    "legalName": "ULC Test XYZ",
                },
                {
                    "identifier": "BC1234567",
                    "entityType": "CC",
                    "legalName": "CCC Test XYZ",
                },
            ],
            None,
            [],
...
def test_alternate_names(
    ...
    for le_info in legal_entities_info:
        ...
        if founding_date_str:
            ...
            founding_date = datetime.utcfromtimestamp(0)
        registration_date = founding_date
        ...
        session.flush()
        assert le.alternate_names

        alternate_names_json = le.alternate_names_json()
        assert alternate_names_json == expected_alternate_names
...
###
#
# New LegalEntity Alternate Names Tests
#
##


@pytest.mark.parametrize(
    "test_name, first_name, middle_initial, last_name, expected",
    [
        ("all elements", "Joe", "F", "Pesci", "Joe F Pesci"),
        ("first & last", "Joe", None, "Pesci", "Joe Pesci"),
        ("mononym", None, None, "Sting", "Sting"),
    ],
)
def test_le_person_names(
    session, test_name, first_name, middle_initial, last_name, expected
):
    """Check the legal_name returned for a Person."""
    with nested_session(session):
        identifier = "P1234567"
        person = person_test_helper(
        ...
        assert person.legal_name == expected


def person_test_helper(
    first_name: str, middle_initial: str, last_name: str, identifier: str = "P1234567"
):
    """Create a person LegalEntity for tesing."""
    person = LegalEntity(
...
def test_sole_proprietor(
    ...
    with nested_session(session):
        ...
        dba = AlternateName(
            ...
            naics_description="pawn stuff",
        )
        person.alternate_names.append(dba)
        person.save()

        # The person's legal name is correct
        assert person
        assert person.legal_name == "Joe F Pesci"

        # The DBA name is correct
        dbas = person.alternate_names.all()
        assert dbas[0].name == "Harry Lime Questionable Goods Broker"

        # The dict for the alternate name is correct
        alternate_names_json = person.alternate_names_json()[0]
        reg_date = datetime.fromisoformat(
            alternate_names_json.pop("nameRegisteredDate", None)
        )
        ...
        assert alternate_names_json == name_dict
        assert reg_date.replace(tzinfo=None) == registration_date

        # The alternate name portion of the LegalEntity is correct
        person_dict = person.json()
        person_dict_alt_name = person_dict["alternateNames"][0]
        del person_dict_alt_name["nameRegisteredDate"]
        assert person_dict["alternateNames"][0] == name_dict
...
def test_find_by_FM_identifier(
    session, test_name, first_name, middle_initial, last_name, expected
):
    with nested_session(session):
        identifier = "P1234567"
        registration_number = "FM1234567"
        registration_date = datetime.utcnow()
        start_date = datetime.utcnow()
        ...
        firm_person = LegalEntity.find_by_identifier(registration_number)

        # The person's legal name is correct
        assert firm_person
        assert firm_person.legal_name == "Joe F Pesci"

        # The DBA name is correct
        dbas = firm_person.alternate_names.all()
        assert dbas[0].name == "Harry Lime Questionable Goods Broker"

        # The dict for the alternate name is correct
        alternate_names_json = firm_person.alternate_names_json()[0]
        reg_date = datetime.fromisoformat(
            alternate_names_json.pop("nameRegisteredDate", None)
        )
...
def test_find_by_FM_name(
    ...
    with nested_session(session):
        ...
        firm_person = LegalEntity.find_by_operating_name(name)
...
@pytest.mark.parametrize(
    "test_name, business_name, test_people, expected",
    [
        (
            "all elements",
...
def test_simple_general_partnership(
    session, test_name, business_name, test_people, expected
):
    with nested_session(session):
        registration_date = datetime.utcnow()
        start_date = datetime.utcnow()

        business_identifier = "FM0001111"

        people = []
        for test_person in test_people:
            person = person_test_helper(
                first_name=test_person["first_name"],
                middle_initial=test_person["middle_initial"],
                last_name=test_person["last_name"],
                identifier=test_person["identifier"],
            )
        ...
        business = LegalEntity(
            ...
            founding_date=datetime.utcfromtimestamp(0),
            state=LegalEntity.State.ACTIVE,
            legal_name="concatenated name from Filer",
        )
        ...
        business.alternate_names.append(dba)
        business.save()

        # The business's legal name is correct
        assert business
        # assert business.legal_name == business_name

        # The DBA name is correct
        dbas = business.alternate_names.all()
        assert dbas[0].name == business_name
