...
def test_valid_alias_save(session):
    ...
    alias = Alias(alias="ABC Ltd.", type="TRANSLATION", legal_entity_id=legal_entity.id)
    alias.save()
    assert alias.id
...
def test_alias_json(session):
    ...
    alias = Alias(alias="ABC Ltd.", type="TRANSLATION", legal_entity_id=legal_entity.id)
    alias.save()
    alias_json = {"id": str(alias.id), "name": alias.alias, "type": alias.type}
...
def test_find_alias_by_business_and_type(session):
    ...
    legal_entity = factory_legal_entity(identifier)
    alias1 = Alias(
        alias="ABC Ltd.", type="TRANSLATION", legal_entity_id=legal_entity.id
    )
