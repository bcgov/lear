...
from business_model.exceptions.error_messages import ErrorCode
...
def factory_incorporation_filing(
    legal_entity, data_dict, filing_date=FROZEN_DATETIME, effective_date=FROZEN_DATETIME
):
    """Create a filing."""
    filing = Filing()
    filing.legal_entity_id = legal_entity.id
...
def factory_legal_entity_comment(
    legal_entity: LegalEntity = None, comment_text: str = "some text", user: User = None
):
    """Create a comment."""
    if not legal_entity:
...
def factory_offices(
    legal_entity, office_types=[OfficeType.REGISTERED], change_filing=None
):
