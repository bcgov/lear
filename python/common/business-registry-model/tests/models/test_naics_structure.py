...
from business_model import NaicsStructure
...
def test_naics_find_by_search_term(session):
    ...
    results_with_more_than_one_element = [
        result for result in results if len(result.naics_elements) > 0
    ]
    assert len(results_with_more_than_one_element) == 7

    results_with_3_elements = [
        result for result in results if len(result.naics_elements) == 3
    ]
...
def test_exact_match_search_naics(app, session, client):
    ...
    # test
    results = NaicsStructure.find_by_search_term(
...
def test_non_exact_match_search_naics(session, client):
    ...
    # verify elements are filtered correctly
    results_with_11_elements = [
        result for result in results if len(result.naics_elements) == 11
    ]
    assert len(results_with_11_elements) == 1

    results_with_2_elements = [
        result for result in results if len(result.naics_elements) == 2
    ]
    assert len(results_with_2_elements) == 1

    results_with_5_elements = [
        result for result in results if len(result.naics_elements) == 5
    ]
