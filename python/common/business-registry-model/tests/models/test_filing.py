...
from business_model.models import Filing, LegalEntity, User
...
def test_filing_json(session):
    ...
    assert (
        filing.json["filing"]["annualReport"] == ANNUAL_REPORT["filing"]["annualReport"]
    )
...
def test_filing_dump_json(session):
    ...
    assert (
        filings.json["filing"]["annualReport"]
        == ANNUAL_REPORT["filing"]["annualReport"]
    )
...
def test_get_filings_by_status_before_go_live_date(
    session, test_type, days, expected, status
):
    """Assert that a filing can be retrieved by status."""
    import copy
    ...
    go_live_date = datetime.date.fromisoformat(GO_LIVE_DATE)
    ...
    rv = Filing.get_filings_by_status(
        legal_entity.id, [Filing.Status.COMPLETED.value], go_live_date
    )
...
def test_get_a_businesses_most_recent_filing_of_a_type(session):
    ...
    b = factory_legal_entity(identifier)
    ar = copy.deepcopy(ANNUAL_REPORT)
    base_ar_date = datetime.datetime(
        2001, 8, 5, 7, 7, 58, 272362, tzinfo=datetime.timezone.utc
    )
    filings = []
    for i in range(0, 5):
...
def test_alteration_filing_with_court_order(session):
    ...
    b = factory_legal_entity(
        identifier, datetime.datetime.utcnow(), None, LegalEntity.EntityTypes.COMP.value
    )
    ...
    assert filing.id is not None
    assert (
        filing.json["filing"]["alteration"]["courtOrder"]["fileNumber"]
        == COURT_ORDER["fileNumber"]
    )
    assert (
        filing.json["filing"]["alteration"]["courtOrder"]["orderDate"]
        == COURT_ORDER["orderDate"]
    )
    assert (
        filing.json["filing"]["alteration"]["courtOrder"]["effectOfOrder"]
        == COURT_ORDER["effectOfOrder"]
    )
...
def test_validate_invalid_court_orders(session, invalid_court_order):
    ...
    with pytest.raises((DataError, DatabaseError)) as excinfo:
        filing.save()
