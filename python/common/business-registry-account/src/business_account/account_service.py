...
class AccountService:
    ...
    @TODO:
      - Add init (set auth, jwt, etc. vars)
      - Add auth api call wrapper (see legal_api/services/authz.py).
      - Cache and refresh / retry token as needed to reduce calls.
    """

    BEARER: ClassVar[str] = "Bearer "
    CONTENT_TYPE_JSON: ClassVar[dict[str, str]] = {"Content-Type": "application/json"}

    jwt: ClassVar[JwtManager | None] = None
    ...
    @classmethod
    def create_affiliation(cls,  # noqa: PLR0913
                           account: int,
    ...
    @classmethod
    def get_account_by_affiliated_identifier(cls, identifier: str, flags: Any | None = None):
    ...
    @classmethod
    def get_contacts(cls, org_id: str, user_token: str | None = None):
        ...
        if user_token:
            ...
            try:
                if not cls.jwt:
                    cls.jwt = JwtManager(current_app)
                token = cls.jwt.get_token_auth_header()
            except Exception:
                token = cls.get_bearer_token()
        else:
            token = cls.get_bearer_token()
