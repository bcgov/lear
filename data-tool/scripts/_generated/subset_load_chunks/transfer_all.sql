...
-- corporation
transfer colin_extract.corporation from ctst using
with corp_list as (
...
-- event
transfer colin_extract.event from ctst using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where c.CORP_NUM in (
...
-- corp_name
transfer colin_extract.corp_name from ctst using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where c.CORP_NUM in (
...
-- corp_state
transfer colin_extract.corp_state from ctst using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where c.CORP_NUM in (
...
-- filing
transfer colin_extract.filing from ctst using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where c.CORP_NUM in (
...
-- filing_user
transfer colin_extract.filing_user from ctst using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where c.CORP_NUM in (
...
transfer colin_extract.subset_address_stage from ctst using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where c.CORP_NUM in (
...
INSERT INTO colin_extract.address (
	addr_id,
	province,
	country_typ_cd,
	postal_cd,
...
FROM (
	SELECT DISTINCT ON (addr_id)
		   ...
		   addr_line_3,
		   city
	FROM colin_extract.subset_address_stage
...
-- office
transfer colin_extract.office from ctst using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where c.CORP_NUM in (
...
-- corp_comments
transfer colin_extract.corp_comments from ctst using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where c.CORP_NUM in (
...
-- ledger_text
transfer colin_extract.ledger_text from ctst using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where c.CORP_NUM in (
...
-- corp_party
transfer colin_extract.corp_party from ctst using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where c.CORP_NUM in (
...
transfer colin_extract.corp_party_relationship from ctst using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where c.CORP_NUM in (
...
-- offices_held
transfer colin_extract.offices_held from ctst using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where c.CORP_NUM in (
...
-- completing_party
transfer colin_extract.completing_party from ctst using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where c.CORP_NUM in (
...
-- submitting_party
transfer colin_extract.submitting_party from ctst using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where c.CORP_NUM in (
...
-- corp_flag
transfer colin_extract.corp_flag from ctst using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where c.CORP_NUM in (
...
-- cont_out
transfer colin_extract.cont_out from ctst using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where c.CORP_NUM in (
...
-- conv_event
transfer colin_extract.conv_event from ctst using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where c.CORP_NUM in (
...
-- conv_ledger
transfer colin_extract.conv_ledger from ctst using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where c.CORP_NUM in (
...
transfer colin_extract.corp_involved_amalgamating from ctst using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where c.CORP_NUM in (
...
transfer colin_extract.corp_involved_cont_in from ctst using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where c.CORP_NUM in (
...
-- corp_restriction
transfer colin_extract.corp_restriction from ctst using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where c.CORP_NUM in (
...
-- correction
transfer colin_extract.correction from ctst using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where c.CORP_NUM in (
...
transfer colin_extract.jurisdiction from ctst using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where c.CORP_NUM in (
...
-- resolution
transfer colin_extract.resolution from ctst using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where c.CORP_NUM in (
...
-- share_struct
transfer colin_extract.share_struct from ctst using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where c.CORP_NUM in (
...
-- share_struct_cls
transfer colin_extract.share_struct_cls from ctst using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where c.CORP_NUM in (
...
-- share_series
transfer colin_extract.share_series from ctst using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where c.CORP_NUM in (
...
-- notification
transfer colin_extract.notification from ctst using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where c.CORP_NUM in (
...
-- notification_resend
transfer colin_extract.notification_resend from ctst using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where c.CORP_NUM in (
...
-- party_notification
transfer colin_extract.party_notification from ctst using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where c.CORP_NUM in (
...
-- payment
transfer colin_extract.payment from ctst using
with corp_list as (
	select /*+ materialize */ c.corp_num
	from corporation c
	where c.CORP_NUM in (
