...
WHERE NOT EXISTS (SELECT 1 FROM colin_extract.event e WHERE e.event_id = t.event_id);
...
WHERE t.event_id IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM colin_extract.event e WHERE e.event_id = t.event_id);
...
WHERE NOT EXISTS (SELECT 1 FROM colin_extract.corp_party cp WHERE cp.corp_party_id = t.party_id);

DELETE FROM colin_extract.offices_held t
WHERE NOT EXISTS (SELECT 1 FROM colin_extract.corp_party cp WHERE cp.corp_party_id = t.corp_party_id);
