...
CREATE OR REPLACE FUNCTION colin_extract.dbcli_varchar_to_boolean(val varchar)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
STRICT
...
CREATE OR REPLACE FUNCTION colin_extract.dbcli_bpchar_to_boolean(val bpchar)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
STRICT
...
CREATE CAST (varchar AS boolean)
    WITH FUNCTION colin_extract.dbcli_varchar_to_boolean(varchar)
...
CREATE CAST (bpchar AS boolean)
    WITH FUNCTION colin_extract.dbcli_bpchar_to_boolean(bpchar)
    AS IMPLICIT -- DbSchemaCLI workaround: avoid keyword being last token
;
