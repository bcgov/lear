-- Delete a chunk of corps FROM TARGET_SCHEMA.the TARGET Postgres extract DB.
--
...
--
-- Intended to be executed FROM TARGET_SCHEMA.a master DbSchemaCLI script connected to the target Postgres DB (cprd_pg).
--
...
-- reloaded FROM TARGET_SCHEMA.Oracle.
-- IMPORTANT:
...
WHERE event_id IN (SELECT event_id FROM TARGET_SCHEMA.event WHERE corp_num IN (&corp_ids_in));
...
WHERE party_id IN (SELECT corp_party_id FROM TARGET_SCHEMA.corp_party WHERE corp_num IN (&corp_ids_in));

DELETE FROM TARGET_SCHEMA.offices_held
WHERE corp_party_id IN (SELECT corp_party_id FROM TARGET_SCHEMA.corp_party WHERE corp_num IN (&corp_ids_in));
