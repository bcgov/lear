...
from .filing_components.filings import update_filing_court_order
from .filing_components import (
    aliases,
    business_info,
    shares,
)
from ..filing_processors.filing_components.offices import update_offices
from ..filing_processors.filing_components.parties import update_parties
...
def _process_corps_conversion(business, conversion_filing, filing, filing_rec):
    if business:
        print(f'Business Already Exist: CONVL legal_filing:conversion {filing_rec.id}')
    if not (corp_num := filing.get('filing', {}).get('business', {}).get('identifier')):
        print(f'conversion {filing_rec.id} missing the business idnetifier.')
    ...
    if not (business := business_info.update_business_info(corp_num, Business(), business_info_obj, filing_rec)):
        print(f'CONVL conversion {filing_rec.id}, Unable to create business.')
