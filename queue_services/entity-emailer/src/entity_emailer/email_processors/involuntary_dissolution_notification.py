...
def _get_pdfs(
    ...
    """Get the pdfs for the involuntary dissolution output."""
    pdfs = []
    attach_order = 1
    headers = {
    ...
    if status == Filing.Status.PAID.value:
        ...
        if legal_type not in ['SP', 'GP']:
            ...
            if filing_pdf.status_code != HTTPStatus.OK:
                ...
                pdfs.append(
                    {
                        'fileName': 'Involuntary Dissolution Application.pdf',
        ...
        if legal_type in ['SP', 'GP']:
            ...
            if filing_pdf.status_code != HTTPStatus.OK:
                ...
                pdfs.append(
                    {
                        'fileName': 'Statement of Involuntary Dissolution.pdf',
                        'fileBytes': filing_pdf_encoded.decode('utf-8'),
                        'fileUrl': '',
                        'attachOrder': str(attach_order)
                    }
            ...
            if filing.filing_sub_type != 'administrative':
                ...
                if certificate.status_code != HTTPStatus.OK:
                    ...
                    pdfs.append(
                        {
                            'fileName': 'Certificate of Involuntary Dissolution.pdf',
...
def process(email_info: dict, token: str) -> dict:  # pylint: disable=too-many-locals, , too-many-branches
    ...
    if status == Filing.Status.PAID.value:
        if legal_type in ['SP', 'GP']:
            ...
            subject = 'InVoluntary dissolution'

    elif status == Filing.Status.COMPLETED.value:
        if legal_type in ['SP', 'GP']:
            subject = 'Involuntary Dissolution Documents from the Business Registry'
        else:
            subject = 'Confirmation of Involuntary Dissolution from the Business Registry'
