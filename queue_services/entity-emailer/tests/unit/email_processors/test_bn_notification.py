...
def test_bn_move_notificaton(app, session):
    ...
    filing = prep_registration_filing(
        session, identifier, "1", "COMPLETED", LegalEntity.EntityTypes.SOLE_PROP.value, "test business"
    )
    token = "token"
    business = LegalEntity.find_by_identifier(identifier)
    # sanity check
    ...
    with patch.object(bn_notification, "get_recipient_from_auth", return_value="user@email.com"):
        email = bn_notification.process_bn_move(
            {"identifier": identifier, "data": {"oldBn": "993775204BC0001", "newBn": "993777399BC0001"}}, token
        )
        # check email values
