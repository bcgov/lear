...
from entity_emailer.email_processors import correction_notification, filing_notification
from tests.unit import prep_firm_correction_filing
...
def test_correction_notification(app, session, status, legal_type):
    ...
    with patch.object(correction_notification, '_get_pdfs', return_value=[]) as mock_get_pdfs:
        ...
        if status == 'PAID':
            ...
            assert email['content']['subject'] == \
                legal_name + ' - Correction of Registration Documents from the Business Registry'
