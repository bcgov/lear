...
from entity_queue_common.version import __version__
...
class Probes():
    ...
    async def meta_handler(self, request):  # pylint: disable=unused-argument; framework callback
        """Version of the probe and service worker."""
        ver = {'probe': __version__}
        if self.components:
            for component in self.components:
                ver[component.__class__.__name__] = component.__version__
                try:
                    ver[component.name] = component.version
    ...
    def get_app(self):
        ...
        if self.app is None:
            ...
            self.app.router.add_route('GET', '/meta', self.meta_handler)
