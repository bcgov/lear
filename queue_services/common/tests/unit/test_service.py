...
async def test_is_healthy(test_name, mock_return, expected):
    ...
    service = ServiceWorker(loop=None, cb_handler=None, config=config.get_named_config())
...
async def test_service_connect(stan_server, event_loop):
    ...
    # setup
    service = ServiceWorker(loop=event_loop, cb_handler=None, config=config.get_named_config())
