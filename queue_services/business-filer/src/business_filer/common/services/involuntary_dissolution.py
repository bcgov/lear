...
class InvoluntaryDissolutionService:
    ...
    def check_business_eligibility(
        cls, identifier: str, eligibility_filters: EligibilityFilters | None
        ...
        """
        defined_filters = eligibility_filters or cls.EligibilityFilters()
        query = cls._get_businesses_eligible_query(defined_filters).filter(Business.identifier == identifier)
    ...
    @staticmethod
    def _get_businesses_eligible_query(eligibility_filters: EligibilityFilters | None): 
        ...
        """
        defined_filters = eligibility_filters or InvoluntaryDissolutionService.EligibilityFilters()
        ...
        future_effective_filing = False if defined_filters.exclude_future_effective_filing \
            else _has_future_effective_filing()
        if defined_filters.exclude_in_dissolution:
                ...
                not_(
                    exists().where(
                        and_(
                            ...
                            == FilingTypes.TRANSITION.value,  # pylint: disable=protected-access
