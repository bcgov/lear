...
del COOP_INCORPORATION_FILING_TEMPLATE["filing"]["incorporationApplication"]["offices"][
    "recordsOffice"
]
del COOP_INCORPORATION_FILING_TEMPLATE["filing"]["incorporationApplication"]["parties"][
    1
]
del COOP_INCORPORATION_FILING_TEMPLATE["filing"]["incorporationApplication"][
    "shareStructure"
]
del COOP_INCORPORATION_FILING_TEMPLATE["filing"]["incorporationApplication"][
    "incorporationAgreement"
]
COOP_INCORPORATION_FILING_TEMPLATE["filing"]["incorporationApplication"]["nameRequest"][
    "legalType"
] = "CP"
COOP_INCORPORATION_FILING_TEMPLATE["filing"]["incorporationApplication"][
    "cooperative"
] = {
...
def test_incorporation_filing_process_with_nr(
    app, session, legal_type, filing, next_corp_num
):
...
def test_incorporation_filing_process_no_nr(
    app, session, legal_type, filing, legal_name_suffix
):
    """Assert that the incorporation object is correctly populated to model objects."""
    # setup
    ...
    with patch.object(
        ...
        # Parties
        parties = filing_rec.filing_json["filing"]["incorporationApplication"][
            "parties"
        ]
...
def test_incorporation_filing_bc_company_from_colin(
    app, session, legal_type, legal_name_suffix
):
