...
def test_worker_continuation_out(app, session):
    ...
    continuation_out_filing = create_filing(
        payment_id, filing_json, business_id=business.id
    )
    ...
    continuation_out.process(
        business, continuation_out_filing, filing_json["filing"], filing_meta
    )
    business.save()

    # Check outcome
    final_filing = Filing.find_by_id(continuation_out_filing.id)
    foreign_jurisdiction_json = filing_json["filing"]["continuationOut"][
        "foreignJurisdiction"
    ]
    continuation_out_date_str = filing_json["filing"]["continuationOut"][
        "continuationOutDate"
    ]
    ...
    assert (
        filing_json["filing"]["continuationOut"]["courtOrder"]["fileNumber"]
        == final_filing.court_order_file_number
    )
    assert (
        filing_json["filing"]["continuationOut"]["courtOrder"]["effectOfOrder"]
        == final_filing.court_order_effect_of_order
    )
    assert (
        filing_json["filing"]["continuationOut"]["details"]
        == final_filing.comments[0].comment
    )
    ...
    assert (
        business.foreign_jurisdiction_region
        == foreign_jurisdiction_json["region"].upper()
    )
    assert (
        business.foreign_legal_name
        == filing_json["filing"]["continuationOut"]["legalName"]
    )
    assert business.continuation_out_date == continuation_out_date
    assert business.dissolution_date == continuation_out_date

    assert (
        filing_meta.continuation_out["country"] == foreign_jurisdiction_json["country"]
    )
    assert filing_meta.continuation_out["region"] == foreign_jurisdiction_json["region"]
    assert (
        filing_meta.continuation_out["continuationOutDate"] == continuation_out_date_str
    )
    assert (
        filing_meta.continuation_out["legalName"]
        == filing_json["filing"]["continuationOut"]["legalName"]
    )
