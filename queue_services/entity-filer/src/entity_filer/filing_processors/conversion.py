...
def process(legal_entity: LegalEntity,  # pylint: disable=too-many-branches
            filing: Dict,
            filing_rec: Filing,
...
def _process_corps_conversion(legal_entity, conversion_filing, filing, filing_rec):
    ...
    if not (legal_entity := legal_entity_info.update_legal_entity_info(corp_num, LegalEntity(), legal_entity_info_obj, filing_rec)):
...
def _process_firms_conversion(legal_entity: LegalEntity, conversion_filing: Dict, filing_rec: Filing, filing_meta: FilingMeta):
    ...
    with suppress(IndexError, KeyError, TypeError):
        ...
        if name_request_json.get('legalName'):
            ...
            legal_entity_info.set_legal_name(legal_entity.identifier, legal_entity, name_request_json)
    ...
    if (naics := conversion_filing.get('filing', {}).get('conversion', {}).get('legal_entity', {}).get('naics')) and \
    ...
    with suppress(IndexError, KeyError, TypeError):
        ...
        if legal_entity_start_date:
            legal_entity.start_date = LegislationDatetime.as_utc_timezone_from_legislation_date_str(legal_entity_start_date)
