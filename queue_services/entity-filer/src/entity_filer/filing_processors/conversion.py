...
def process(business: Business, filing: Dict, filing_rec: Filing):  # pylint: disable=too-many-branches
    ...
    conversion_filing = filing.get('filing', {}).get('conversion')
    is_correction = filing_rec.filing_type == 'correction'
    if not conversion_filing:
    ...
    if is_correction:
        ...
        if filing_rec.colin_event_ids:
            ...
            if not corp_num:
                raise QueueException(
                    f'conversion {filing_rec.id} unable to get a business registration number.')
    ...
    if offices := conversion_filing['offices']:
