        ...
        try:
            for party_info in parties_structure:
                ...
                for role_type in party_info.get('roles'):
                    ...
                    if party_role.role in [EntityRole.RoleTypes.completing_party.value,
...
def merge_all_parties (legal_entity: LegalEntity,
                       filing: Filing,
                       parties: dict
    ) -> [dict] | None:
    ...
    for party_dict in parties['parties']:
        ...
        if not (party_type := party_dict.get('officer', {}).get('partyType')):
            ...
            raise BusinessException(status_code=status_code,
        ...
        party_le = None
        if ((party_identifier := party_dict.get('officer', {}).get('identifier'))
           and (
               (party_le := LegalEntity.find_by_identifier (party_identifier))
             or
               (party_le := ColinEntity.find_by_identifier (party_identifier))
            )
        )or(
        ...
        if existing_party and party_type == 'person':
            ...
            mailing_address = get_address_for_filing(
                                    party_le.entity_mailing_address,
                                    party_dict.get('mailingAddress'))
        else:
    ...
    if memoize_existing_director_ids:
        delete_non_memoized_directors(legal_entity,
                                      filing,
                                      memoize_existing_director_ids)
...
def merge_entity_role_to_filing(party_le: LegalEntity,
                                   filing: Filing,
    ...
    """
    if not (base_entity
            and (entity_role := EntityRole.find_by_party_id_and_role(
...
def merge_entity_role_for_director(party_le: LegalEntity,
    ...
    """Add the EntityRole to the filing.
    """
    if not (entity_role := EntityRole.find_by_party_id_and_role(
        party_id=party_le.id,
        legal_entity_id=base_entity.id,
...
def get_address_for_filing(party_address: Address,
                           address_dict: dict
) -> Address:
...
def delete_non_memoized_directors(legal_entity: LegalEntity,
                                  filing: Filing,
                                  keep_list,
) -> []:
    """Delete EntityRoles for directors not in the keep_list."""
    candidates = EntityRole.get_parties_by_role(legal_entity.id,
