...
def set_legal_name(corp_num: str, legal_entity: LegalEntity, legal_entity_info: Dict):
    ...
    legal_name = legal_entity_info.get('legalName', None)
    if legal_name:
        legal_entity.legal_name = legal_name
    else:
        entity_type = legal_entity_info.get('legalType', None)
        numbered_legal_name_suffix = LegalEntity.BUSINESSES[entity_type]['numberedLegalNameSuffix']
...
def get_next_corp_num(entity_type: str):
    ...
    try:
        ...
        if entity_type in (LegalEntity.EntityTypes.BCOMP.value,
