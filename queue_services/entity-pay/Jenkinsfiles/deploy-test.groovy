        ...
        script {
            try {
                timeout(time: 1, unit: 'DAYS') {
                    input message: "Run E2E pipeline?", id: "1234", submitter: 'admin,thorwolpert-admin,rarmitag-admin,kialj876-admin,katiemcgoff-admin,WalterMoar-admin,JohnamLane-edit'
                }
                openshift.withCluster() {
    ...
    stage("Verify E2E Tests") {
        script {
            if (!passed) {
                try {
                    timeout(time: 1, unit: 'DAYS') {
                        input message: "E2E failed or were not run. Proceed to test?", id: "1234", submitter: 'admin,thorwolpert-admin,rarmitag-admin,kialj876-admin,katiemcgoff-admin,WalterMoar-admin,JohnamLane-edit'
                    }
                } catch (Exception e1) {
                    try {
                        timeout(time: 1, unit: 'DAYS') {
                            input message: "Keep E2E image?", id: "1234", submitter: 'admin,thorwolpert-admin,rarmitag-admin,kialj876-admin,katiemcgoff-admin,WalterMoar-admin,JohnamLane-edit'
                        }
                    } catch (Exception e2) {
