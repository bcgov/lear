...
def prep_registration_filing(
    session, identifier, payment_id, option, legal_type, legal_name
):
    ...
    gp_registration["filing"]["registration"]["parties"][1]["officer"][
        "email"
    ] = "party@email.com"
    ...
    filing = create_filing(
        token=payment_id, filing_json=filing_template, business_id=business_id
    )
    filing.payment_completion_date = filing.filing_date
    filing.save()
...
def prep_consent_amalgamation_out_filing(
    session, identifier, payment_id, legal_type, legal_name, submitter_role
):
    ...
    filing_template["filing"]["consentAmalgamationOut"] = copy.deepcopy(
        CONSENT_AMALGAMATION_OUT
    )
    filing_template["filing"]["business"] = {
        "identifier": business.identifier,
...
def prep_amalgamation_out_filing(
    session, identifier, payment_id, legal_type, legal_name, submitter_role
):
...
def prep_consent_continuation_out_filing(
    session, identifier, payment_id, legal_type, legal_name, submitter_role
):
...
def prep_continuation_out_filing(
    session, identifier, payment_id, legal_type, legal_name, submitter_role
):
...
def prep_restoration_filing(
    identifier, payment_id, legal_type, legal_name, r_type="fullRestoration"
):
...
def prep_change_of_registration_filing(
    session,
    identifier,
    payment_id,
    legal_type,
...
def prep_alteration_filing(session, identifier, option, company_name):
    ...
    business = create_business(
        identifier, legal_type=Business.LegalTypes.BCOMP.value, legal_name=company_name
    )
...
def prep_maintenance_filing(
    session, identifier, payment_id, status, filing_type, submitter_role=None
):
...
def prep_incorporation_correction_filing(
    session, business, original_filing_id, payment_id, option
):
...
def prep_firm_correction_filing(
    session,
    identifier,
    payment_id,
    legal_type,
    ...
    gp_correction["filing"]["correction"]["parties"][0]["officer"][
        "email"
    ] = "party@email.com"

    sp_correction = copy.deepcopy(CORRECTION_REGISTRATION)
    sp_correction["filing"]["correction"]["parties"][0]["officer"][
        "email"
    ] = "party@email.com"
...
def prep_cp_special_resolution_filing(
    identifier, payment_id, legal_type, legal_name, submitter_role=None
):
...
def prep_cp_special_resolution_correction_upload_memorandum_filing(
    ...
    filing_template["filing"]["correction"][
        "memorandumFileKey"
    ] = "28f73dc4-8e7c-4c89-bef6-a81dff909ca6.pdf"
...
def prep_amalgamation_filing(session, identifier, payment_id, option, legal_name):
    """Return a new incorp filing prepped for email notification."""
    business = create_business(
        identifier, legal_type=Business.LegalTypes.BCOMP.value, legal_name=legal_name
    )
    filing_template = copy.deepcopy(FILING_HEADER)
    filing_template["filing"]["header"]["name"] = "amalgamationApplication"

    filing_template["filing"]["amalgamationApplication"] = copy.deepcopy(
        AMALGAMATION_APPLICATION
    )
    ...
    for party in filing_template["filing"]["amalgamationApplication"]["parties"]:
        for role in party["roles"]:
            if role["roleType"] == "Completing Party":
                party["officer"]["email"] = "comp_party@email.com"
    filing_template["filing"]["amalgamationApplication"]["contactPoint"][
        "email"
    ] = "test@test.com"
...
def prep_continuation_in_filing(session, identifier, payment_id, option):
    """Return a new incorp filing prepped for email notification."""
    business = create_business(
        identifier, legal_type=Business.LegalTypes.BCOMP_CONTINUE_IN.value
    )
    filing_template = copy.deepcopy(CONTINUATION_IN_FILING_TEMPLATE)
    if business.legal_type:
        filing_template["filing"]["continuationIn"]["nameRequest"][
            "legalType"
        ] = business.legal_type
    for party in filing_template["filing"]["continuationIn"]["parties"]:
        for role in party["roles"]:
            if role["roleType"] == "Completing Party":
                party["officer"]["email"] = "comp_party@email.com"
    filing_template["filing"]["continuationIn"]["contactPoint"][
        "email"
    ] = "test@test.com"
...
def prep_intent_to_liquidate_filing(
    session, identifier, payment_id, legal_type, legal_name, submitter_role
):
    ...
    filing_template["filing"]["intentToLiquidate"][
    ...
    test_meta_data = {
        "intentToLiquidate": {"dateOfCommencementOfLiquidation": future_date}
    }
...
def prep_notice_of_withdraw_filing(
    identifier, payment_id, legal_type, legal_name, business_id, withdrawn_filing
):
    ...
    filing_template["filing"]["noticeOfWithdrawal"] = copy.deepcopy(
        NOTICE_OF_WITHDRAWAL
    )
...
def create_future_effective_filing(
    ...
    if is_temp:
        ...
        filing_template["filing"][filing_type]["contactPoint"][
            "email"
        ] = "recipient@email.com"
    else:
