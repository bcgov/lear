...
@patch.object(restoration, 'replace_digital_credential')
def test_processor_returns_none_if_no_issued_credentials(mock_replace_digital_credential,
...
@patch.object(restoration, 'replace_digital_credential')
def test_processor_replaces_all_credentials(mock_replace_digital_credential,
                                          mock_get_all_digital_credentials_for_business,
                                          app, session):
    """Assert that the processor replaces all credentials when business is restored."""
    business = create_business(identifier='FM0000001')
    ...
    mock_get_all_digital_credentials_for_business.assert_called_once_with(business=business)
    assert mock_replace_digital_credential.call_count == 2
    # Check that replace_digital_credential was called with correct arguments for each credential
    mock_replace_digital_credential.assert_any_call(
...
@patch.object(restoration, 'get_all_digital_credentials_for_business')
@patch.object(restoration, 'replace_digital_credential')
def test_processor_returns_none_with_empty_credentials_list(mock_replace_digital_credential,
                                                           mock_get_all_digital_credentials_for_business,
                                                           app, session):
...
@patch.object(restoration, 'get_all_digital_credentials_for_business')
@patch.object(restoration, 'replace_digital_credential')
def test_processor_returns_none_with_none_credentials(mock_replace_digital_credential,
                                                     mock_get_all_digital_credentials_for_business,
                                                     app, session):
