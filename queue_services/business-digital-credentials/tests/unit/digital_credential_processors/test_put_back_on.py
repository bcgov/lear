...
from business_digital_credentials.digital_credential_processors import put_back_on
from tests.unit import create_business


@patch.object(put_back_on, "get_all_digital_credentials_for_business", return_value=[])
@patch.object(put_back_on, "replace_digital_credential")
def test_processor_does_not_run_if_no_issued_credential(
    mock_replace_digital_credential, mock_get_issued_digital_credentials, app, session
):
...
@patch.object(
    put_back_on, "get_all_digital_credentials_for_business", return_value=[{"id": 1}]
)
@patch.object(put_back_on, "replace_digital_credential")
def test_processor_replaces_issued_credential(
