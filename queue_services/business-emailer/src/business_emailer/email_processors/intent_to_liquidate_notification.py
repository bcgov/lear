...
"""Email processing rules and actions for Intent to Liquidate notifications."""
import base64
import re
...
def process(email_info: dict, token: str) -> dict:   # pylint: disable=too-many-locals
    """Build the email for Intent to Liquidate notification."""
    current_app.logger.debug('intent_to_liquidate_notification: %s', email_info)
    ...
    template = Path(
        f'{current_app.config.get("TEMPLATE_PATH")}/INTENTTOLIQUIDATE-COMPLETED.html'
    ).read_text()
    ...
    if filing.submitter_roles and UserRoles.staff in filing.submitter_roles:
        # when staff file a documentOptionalEmail may contain completing party email
...
def _get_pdfs(
    ...
    """Get the PDFs for the Statement of Intent to Liquidate output."""
    pdfs = []
    attach_order = 1
    headers = {
