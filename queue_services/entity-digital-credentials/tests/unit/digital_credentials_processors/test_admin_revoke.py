...
from entity_digital_credentials.digital_credentials_processors.admin_revoke import process
from tests.unit import create_business


@pytest.mark.asyncio
@patch('entity_digital_credentials.digital_credentials_processors.admin_revoke.get_issued_digital_credentials',
       return_value=[])
@patch('entity_digital_credentials.digital_credentials_processors.admin_revoke.logger')
@patch('entity_digital_credentials.digital_credentials_processors.admin_revoke.revoke_digital_credential')
...
async def test_processor_revokes_issued_credential(mock_revoke_digital_credential,
    ...
    mock_revoke_digital_credential.assert_called_once_with(
        issued_credential={'id': 1}, reason=DCRevocationReason.ADMINISTRATIVE_REVOCATION)
